...
HRESULT CAudioSynth::AllocWaveCache(const WAVEFORMATEX& wfex)
{
    ...
    int iSize = MulDiv(m_iWaveCacheSize, wfex.wBitsPerSample, 8);
...
void CAudioSynth::GetPCMFormatStructure(WAVEFORMATEX* pwfex)
{
    ...
    ASSERT((8 == m_wBitsPerSample) || (16 == m_wBitsPerSample) || (24 == m_wBitsPerSample));
...
void CAudioSynth::FillPCMAudioBuffer(const WAVEFORMATEX& wfex, BYTE pBuf[], int iSize)
{
   ...
   else if(wfex.wBitsPerSample == 24 && wfex.nChannels == 1)
   {
   ...
   else if(wfex.wBitsPerSample == 24 && wfex.nChannels == 2)
   {
      BYTE *pW      = (BYTE *) pBuf, 
...
void CAudioSynth::CalcCacheSine(const WAVEFORMATEX& wfex)
{
   ...
   double FTwoPIDivSpS;
   int iSamplesPerSine = (int)((double)wfex.nSamplesPerSec / (double)m_iFrequency);
   if (iSamplesPerSine < 1) iSamplesPerSine = 1;
   m_iWaveCacheEnd = iSamplesPerSine * m_iFrequency;

   FTwoPIDivSpS = TWOPI*(double)m_iFrequency / (double)m_iWaveCacheEnd;

   d = (double)wfex.nSamplesPerSec / (double)m_iFrequency;
   d = TWOPI  / d;
   ...
   switch (wfex.wBitsPerSample)
   {
      case 8:
      {
         amplitude  = 127.0 * (double)m_iAmplitude / 1000.0;
         amplitude2 = 127.0 * (double)m_iAmplitude2 / 1000.0;
         offset     = 0.5 + 127.0 * (double)m_iOffset1 / 1000.0;
         offset2    = 0.5 + 127.0 * (double)m_iOffset2 / 1000.0;
         PBYTE pW = (PBYTE) m_bWaveCache;
         PBYTE pW2 = (PBYTE) m_bWaveCache2;
         for(i=0,d=0; i < m_iWaveCacheEnd; i++)
         {
            dtmp = sin(d);
            *pW++  = (BYTE) (dtmp * amplitude  + offset);
            *pW2++ = (BYTE) (dtmp * amplitude2 + offset2);
            d += FTwoPIDivSpS;
            if (d>TWOPI) d-= TWOPI;
         }
      }break;
      case 16:
      {
         amplitude  = 32767.0 * (double)m_iAmplitude / 1000.0;
         amplitude2 = 32767.0 * (double)m_iAmplitude2 / 1000.0;
         offset     = 0.5 + 32767.0 * (double)m_iOffset1 / 1000.0;
         offset2    = 0.5 + 32767.0 * (double)m_iOffset2 / 1000.0;
         PWORD pW  = (PWORD) m_bWaveCache;
         PWORD pW2 = (PWORD) m_bWaveCache2;
         for(i=0,d=0; i < m_iWaveCacheEnd; i++)
         {
            dtmp = sin(d);
            *pW++  = (WORD) (dtmp * amplitude  + offset );
            *pW2++ = (WORD) (dtmp * amplitude2 + offset2);
            d += FTwoPIDivSpS;
            if (d>TWOPI) d-= TWOPI;
         }
      }break;
      case 24:
      {
         amplitude  = 8388607.0 * (double)m_iAmplitude / 1000.0;
         amplitude2 = 8388607.0 * (double)m_iAmplitude2 / 1000.0;
         offset     = 8388607.5 + 8388607.0 * (double)m_iOffset1 / 1000.0;
         offset2    = 8388607.5 + 8388607.0 * (double)m_iOffset2 / 1000.0;
         ...
         {
            dtmp  = sin(d);
            nsin  = (int) (dtmp * amplitude  + offset);
            memcpy(pW, &nsin, 3);
            pW += 3;
            nsin  = (int) (dtmp * amplitude2 + offset2);
            memcpy(pW2, &nsin, 3);
            pW2 += 3;
            d += FTwoPIDivSpS;
            if (d>TWOPI) d-= TWOPI;
...
void CAudioSynth::CalcCacheSquare(const WAVEFORMATEX& wfex)
{
    ...
    amplitude  = 127.0 * (double)m_iAmplitude / 1000.0;
    offset     = 0.5 + 127.0 * (double)m_iOffset1 / 1000.0;
    b0  = (BYTE) (offset - amplitude);
    b1  = (BYTE) (offset + amplitude);
    amplitude  = 127.0 * (double)m_iAmplitude2 / 1000.0;
    offset     = 0.5 + 127.0 * (double)m_iOffset2 / 1000.0;
    b20 = (BYTE) (offset - amplitude);
    b21 = (BYTE) (offset + amplitude);

    amplitude  = 32767.0 * (double)m_iAmplitude / 1000.0;
    offset     = 32767.0 * (double)m_iOffset1 / 1000.0;
    w0   = (WORD) (offset + amplitude);
    w1   = (WORD) (offset - amplitude);
    amplitude  = 32767.0 * (double)m_iAmplitude2 / 1000.0;
    offset     = 32767.0 * (double)m_iOffset2 / 1000.0;
    w20  = (WORD) (offset + amplitude);
    w21  = (WORD) (offset - amplitude);

    amplitude  = 8388607.0 * (double)m_iAmplitude / 1000.0;
    offset     = 0.5 + 8388607.0 * (double)m_iOffset1/ 1000.0;
    n0  = (int)  (offset + amplitude);
    n1  = (int)  (offset - amplitude);
    amplitude  = 8388607.0 * (double)m_iAmplitude2 / 1000.0;
    offset     = 0.5 + 8388607.0 * (double)m_iOffset2 / 1000.0;
    n20  = (int)  (offset + amplitude);
    n21  = (int)  (offset - amplitude);

    CyclesPerSample = (int)floor((double)wfex.nSamplesPerSec / (double)m_iFrequency);
    m_iWaveCacheEnd = m_iFrequency * CyclesPerSample;
    ...
    if (wfex.wBitsPerSample == 8)
    {
        ...
        BYTE * pB2 = m_bWaveCache2;

        for(i = 0; i < m_iWaveCacheEnd; i++)
        {
            *pB++  = (BYTE) ((n++ < nHigh) ? b0  : b1);
            *pB2++ = (BYTE) ((n++ < nHigh) ? b20 : b21);
    ...
    else if (wfex.wBitsPerSample == 16)
    {
        ...
        for(i = 0; i < m_iWaveCacheEnd; i++)
        {
            *pW++  = (WORD) ((n++ < nHigh) ? w0 : w1);
            *pW2++ = (WORD) ((n++ < nHigh) ? w20 : w21);
            if (n == CyclesPerSample)
               n = 0;
       }
    }
    else if (wfex.wBitsPerSample == 24)
    {
        ...
        for(i = 0; i < m_iWaveCacheEnd; i++)
        {
            dw = ((n < nHigh) ? n0 : n1) + 8388607;
            memcpy(pB, &dw, 3);
            pB += 3;
            dw = ((n < nHigh) ? n20 : n21) + 8388607;
            memcpy(pB2, &dw, 3);
            pB2 += 3;
            if (n++ == CyclesPerSample)
               n = 0;
...
void CAudioSynth::CalcCacheSawtooth(const WAVEFORMATEX& wfex)
{
   ...
   double dtmp;
   double amplitude, amplitude2, offset, offset2;
   double step_up, step_down;
   int iPeriodsPerSecond, iSamplesPerPeriod = (int)((double)wfex.nSamplesPerSec / (double)m_iFrequency);
   if (iSamplesPerPeriod < 1) iSamplesPerPeriod = 1;
   iPeriodsPerSecond = wfex.nSamplesPerSec / iSamplesPerPeriod;
   m_iWaveCacheEnd = iSamplesPerPeriod * iPeriodsPerSecond;
   ...
   switch (wfex.wBitsPerSample)
   {
      case 8:
      {
         ...
         PBYTE pW   = (PBYTE) m_bWaveCache;
         PBYTE pW2  = (PBYTE) m_bWaveCache2;
         for(i=0,dtmp=-1.0; i < m_iWaveCacheEnd; i++)
         {
            n = i % iSamplesPerPeriod;
            *pW++  = (BYTE) (dtmp * amplitude  + offset );
            *pW2++ = (BYTE) (dtmp * amplitude2 + offset2);
            if (n < t1)
            {
      ...
      case 24:
      {
         ...
         amplitude2 = 8388607.0 * (double)m_iAmplitude2 / 1000.0;
         offset     = 0.5 + 8388607.0 * (double)m_iOffset1 / 1000.0;
         offset2    = 0.5 + 8388607.0 * (double)m_iOffset2 / 1000.0;
         PBYTE pW  = m_bWaveCache;
         PBYTE pW2 = m_bWaveCache2;
         int dw;
         for(i=0,dtmp=-1.0; i < m_iWaveCacheEnd; i++)
         {
            n = i % iSamplesPerPeriod;
            dw = (int) (dtmp * amplitude + offset);
            memcpy(pW, &dw, 3);
            pW += 3;
            dw = (int) (dtmp * amplitude2 + offset2);
            memcpy(pW2, &dw, 3);
            pW2 += 3;
            if (n < t1)
            {
...
void CAudioSynth::CalcCacheSweep(const WAVEFORMATEX& wfex)
{
    ...
    DeltaFreq = ((double) m_iSweepEnd - m_iSweepStart) / m_iWaveCacheEnd;
    ...
    if(wfex.wBitsPerSample == 8)
    {
        ...
        for(i = 0; i < m_iWaveCacheEnd; i++)
        {
            ...
            d += FTwoPIDivSpS;
            dtemp = sin(d);
            *pB++  = (BYTE) ((dtemp * amplitude)  + offset);
            *pB2++ = (BYTE) ((dtemp * amplitude2) + offset2);
        }
    }
    else if(wfex.wBitsPerSample == 16)
    {
        ...
        for(i = 0; i < m_iWaveCacheEnd; i++)
        {
            ...
            d += FTwoPIDivSpS;
            dtemp = sin(d);
            *pW++  = (WORD) (dtemp * amplitude  + offset);
            *pW2++ = (WORD) (dtemp * amplitude2 + offset2);
        }
    }
    else
    {
        ...
        for(i = 0; i < m_iWaveCacheEnd; i++)
        {
            ...
            d += FTwoPIDivSpS;
            dtemp = sin(d);
            nsin  = (int) (dtemp * amplitude  + offset );
            memcpy(pW, &nsin, 3);
            pW += 3;
            nsin  = (int) (dtemp * amplitude2 + offset2);
            memcpy(pW2, &nsin, 3);
            pW2 += 3;
        }
    }
