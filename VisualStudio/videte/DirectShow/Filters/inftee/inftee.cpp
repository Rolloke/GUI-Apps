...
{
    &CLSID_InfiniteTee,     // CLSID of filter
...
CTee::CTee(TCHAR *pName, LPUNKNOWN pUnk, HRESULT *phr) :
    ...
    CBaseFilter(NAME("Tee filter"), pUnk, this, CLSID_InfiniteTee)
{
    ASSERT(phr);
...
STDMETHODIMP CTeeInputPin::EnableOutPin(int n, BOOL bEnable)
{
	if (n < 0) return E_INVALIDARG;
	CTeeOutputPin *pPin = m_pTee->GetPinNFromList(n);
	if (pPin)
	{
		pPin->m_bActivated = bEnable;
	   return NOERROR;
	}
   return VFW_E_NOT_FOUND;
}

STDMETHODIMP CTeeInputPin::IsOutPinEnabled(int n, BOOL*pbEnable)
{
	if (n < 0) return E_INVALIDARG;
	...
	if (pPin)
	{
		*pbEnable = pPin->m_bActivated;
	   return NOERROR;
	}
   return VFW_E_NOT_FOUND;
}

STDMETHODIMP CTeeInputPin::SetDataCheckFunction(int n, DATACHECK_FUNCTION pFnc, long lParam)
{
	if (n < 0) return E_INVALIDARG;
	...
	if (pPin)
	{
		pPin->m_pCheckFunc = pFnc;
		pPin->m_lParam     = lParam;
	   return NOERROR;
	}
   return VFW_E_NOT_FOUND;
}
...
STDMETHODIMP CTeeInputPin::NonDelegatingQueryInterface(REFIID riid, void **ppv)
{
   if (riid == IID_IInfiniteTee)
	{
       return GetInterface((IInfTee*) this, ppv);
   }
	else
	{
...
HRESULT CTeeOutputPin::Deliver(IMediaSample *pMediaSample)
{
	...
	if (m_pCheckFunc)
	{
		pMediaSample->SetMediaType(&m_mt);
		if (m_pCheckFunc(pMediaSample, m_lParam) != NOERROR)
		{
			return NOERROR;
		}
	}
