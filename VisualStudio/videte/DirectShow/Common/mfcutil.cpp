...
static const LPCTSTR g_sError = _T("<ERROR>");
static const LPCTSTR g_sNoEntry = _T("<< No entries >>");

HRESULT AddGraphFiltersToList(IGraphBuilder *pGB, CWnd*pWnd) 
{
    HRESULT hr;
    ...
    if (FAILED(hr))
    {
		if (pList ) pList->AddString(g_sError);
		if (pCombo) pCombo->AddString(g_sError);
		return hr;
    }
    ...
    while(pEnum->Next(1, &pFilter, &cFetched) == S_OK)
    {
        ...
        {
			CString sName(FilterInfo.achName);
			if (pList ) pList->AddString(sName);
			if (pCombo) pCombo->AddString(sName);
...
HRESULT EnumFiltersWithCLSIDToList(ICreateDevEnum *pSysDevEnum, const GUID *clsid, CWnd *pWnd)
{
    HRESULT hr;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = EnumFiltersAndCLSIDsToList(pEnumCat, pWnd);
...
HRESULT EnumFiltersWithMonikerToList(ICreateDevEnum *pSysDevEnum, const GUID *clsid, CWnd *pWnd)
{
    HRESULT hr;
    IEnumMoniker *pEnumCat = NULL;
    ...
    {
        // Enumerate all filters using the category enumerator
        hr = EnumFiltersAndMonikersToList(pEnumCat, pWnd);
...
HRESULT EnumFiltersAndMonikersToList(IEnumMoniker *pEnumCat, CWnd *pWnd)
{
    HRESULT hr=S_OK;
    IMoniker *pMoniker;
    ULONG cFetched;
    ...
    if (!pEnumCat)
    {
		if      (pWnd->IsKindOf(RUNTIME_CLASS(CListBox ))) ((CListBox* )pWnd)->AddString(g_sNoEntry);
		else if (pWnd->IsKindOf(RUNTIME_CLASS(CComboBox))) ((CComboBox*)pWnd)->AddString(g_sNoEntry);
    ...
    while(pEnumCat->Next(1, &pMoniker, &cFetched) == S_OK)
    {
		...
		hr = AddFilterToListWithMoniker(str, pMoniker, *pWnd);
        // Cleanup interfaces
...
HRESULT EnumFiltersAndCLSIDsToList(IEnumMoniker *pEnumCat, CWnd*pWnd)
{
    HRESULT hr=S_OK;
    IMoniker *pMoniker;
    ULONG cFetched;
    ...
    {
		if (pWnd->IsKindOf(RUNTIME_CLASS(CListBox))) ((CListBox*)pWnd)->AddString(g_sNoEntry);
		if (pWnd->IsKindOf(RUNTIME_CLASS(CComboBox))) ((CComboBox*)pWnd)->AddString(g_sNoEntry);
		else return E_UNEXPECTED;
        return S_FALSE;
    }
    ...
    while(pEnumCat->Next(1, &pMoniker, &cFetched) == S_OK)
    {
        ...
        if(SUCCEEDED(hr))
        {
            ...
            {
                hr = AddFilterToListWithCLSID(str, &clsidFilter, *pWnd);
            }
...
HRESULT EnumPinsOnFilter (IBaseFilter *pFilter, PIN_DIRECTION PinDir, CWnd* pWnd)
{
	CListBox *pList = NULL;
	CComboBox *pCombo = NULL;
	if (pWnd->IsKindOf(RUNTIME_CLASS(CListBox)))
    ...
    while((hr = pEnum->Next(1, &pPin, 0)) == S_OK)
    {
        ...
        if (PinDir == PinDirThis)
        {
            ...
            if (SUCCEEDED(hr))
            {
				...
				if (pList)  pList->AddString(str);
				if (pCombo) pCombo->AddString(str);
...
HRESULT AddFilterPinsToLists (IGraphBuilder *pGB, LPCTSTR pszFilterName, CWnd& ListPinsInput, CWnd& ListPinsOutput) 
{
    HRESULT hr;
    IBaseFilter *pFilter = NULL;

    // Read the current list box name and find it in the graph
    pFilter = FindFilterFromName(pGB, (LPTSTR)pszFilterName);
    if (!pFilter)
        return E_UNEXPECTED;
    ...
    hr = EnumPinsOnFilter(pFilter, PINDIR_INPUT, &ListPinsInput);

    if (SUCCEEDED(hr))
    {
        //RetailOutput(_T("Listing output pins on filter %s\n"), szNameToFind);
        hr = EnumPinsOnFilter(pFilter, PINDIR_OUTPUT, &ListPinsOutput);
    }
...
//////////////////////////////////////////////////////////////////////////
HRESULT AddEventToList(CWnd& Wnd, long lEventCode)
{
...
#define HANDLE_EC(c)                              \
    ...
    if (bMatch)
	{
		if (Wnd.IsKindOf(RUNTIME_CLASS(CListBox)))
		{
			((CListBox*)&Wnd)->AddString(szMsg);
		}
		else if (Wnd.IsKindOf(RUNTIME_CLASS(CComboBox)))
		{
			((CComboBox*)&Wnd)->AddString(szMsg);
		}
		else return E_UNEXPECTED;
	}
	return S_OK;
...
//////////////////////////////////////////////////////////////////////////
HRESULT AddFilterCategoriesToList(CWnd& ListFilters)
{
    HRESULT hr;
            ...
            if(SUCCEEDED(hr))
            {
                ...
                if(SUCCEEDED(hr))
                {
                    ...
                    if(CLSIDFromString(varCatClsid.bstrVal, &clsidCat) == S_OK)
                    {
                        ...
                        hr = AddFilterToListWithCLSID(szCatDesc, &clsidCat, ListFilters);
						if (FAILED(hr))
							return hr;
                        nClasses++;
                    }
...
//////////////////////////////////////////////////////////////////////////
HRESULT AddFilterToListWithCLSID(const TCHAR *szFilterName, const GUID *pCatGuid, CWnd& Wnd)
{
	...
	if (Wnd.IsKindOf(RUNTIME_CLASS(CListBox)))
	{
		CListBox &ListFilters = (CListBox&)Wnd;
	...
	{
		CComboBox &ListFilters = (CComboBox&)Wnd;
		// Add the category name and a pointer to its CLSID to the list box
		int nSuccess  = ListFilters.AddString(szFilterName);
		int nIndexNew = ListFilters.FindStringExact(-1, szFilterName);
...
//////////////////////////////////////////////////////////////////////////
void ClearFilterListWithCLSID(CWnd& Wnd)
{
...
//////////////////////////////////////////////////////////////////////////
void ClearFilterListWithMoniker(CWnd& Wnd)
{
...
//////////////////////////////////////////////////////////////////////////
HRESULT AddFilterToListWithMoniker(const TCHAR *szFilterName, 
                                IMoniker *pMoniker, CWnd &Wnd)
{
	// Add the category name and a pointer to its CLSID to the list box
	if (Wnd.IsKindOf(RUNTIME_CLASS(CListBox)))
	{
		CListBox &ListFilters = (CListBox&)Wnd;
		int nIndex    = ListFilters.AddString(szFilterName);
		int nIndexNew = ListFilters.FindStringExact(-1, szFilterName);
		int nCount = 0;
		while (ListFilters.GetItemDataPtr(nIndexNew))	// same Name before
		{
			nIndexNew = ListFilters.FindString(nIndexNew, szFilterName);
			nCount++;
		}
		if (nCount)
		{
			TCHAR szFilter[256];
			_stprintf(szFilter, _T("%s (%d)"), szFilterName, nCount);
			ListFilters.DeleteString(nIndex);
			ListFilters.InsertString(nIndex, szFilter);
		}
		nIndex = ListFilters.SetItemDataPtr(nIndexNew, pMoniker);
	}
	else if (Wnd.IsKindOf(RUNTIME_CLASS(CComboBox)))
	{
		CComboBox &ListFilters = (CComboBox&)Wnd;
...
//////////////////////////////////////////////////////////////////////////
HRESULT AddVideoStreamConfigCapsToList(IBaseFilter *pFilter, int nPin, CWnd &Wnd)
{
	AM_MEDIA_TYPE *pmt=NULL;
	int i;
	HRESULT          hr        = NOERROR;
	IPin            *pPinOut   = NULL;
	VIDEO_STREAM_CONFIG_CAPS **ppVScc = NULL;
	CComboBox cCombo;
	if (Wnd.IsKindOf(RUNTIME_CLASS(CComboBox)))
	{
		cCombo.m_hWnd= Wnd.m_hWnd;
	}
	else
	{
		return E_UNEXPECTED;
	...
	ClearVideoStreamConfigCapsList(Wnd);

	hr = GetPin(pFilter, PINDIR_OUTPUT, nPin, &pPinOut);
	if (FAILED(hr))
	{
		return hr;
	}
	...
	if (SUCCEEDED(hr))
	{
		...
		CString  str, sSubtype;
		int  iCount, iSize, nWeight;

		hr = pCfg->GetNumberOfCapabilities(&iCount, &iSize);
		if (SUCCEEDED(hr))
		{
			ppVScc = (VIDEO_STREAM_CONFIG_CAPS**)CoTaskMemAlloc(iCount* sizeof(void*));
			ZeroMemory(ppVScc, iCount* sizeof(void*));
			SetWindowLong(Wnd.m_hWnd, GWL_USERDATA, (long)ppVScc);

			for (i=0; i<iCount; i++)
			{
				nWeight = 0;
				pVSCC = (VIDEO_STREAM_CONFIG_CAPS*) CoTaskMemAlloc(iSize);
				hr = pCfg->GetStreamCaps(i, &pmt, (BYTE*)pVSCC);
				if (FAILED(hr))
				{
				...
				if (sSubtype == _T("NULL")) sSubtype.Empty();
				str.Format(_T("%s (%d x %d)"), sSubtype, pVSCC->InputSize.cx, pVSCC->InputSize.cy);
				cCombo.AddString(str);
				cCombo.SetItemData(i, (ULONG)pmt);
				ppVScc[i] = pVSCC;
			}
			RELEASE_OBJECT(pCfg);
		}
...
//////////////////////////////////////////////////////////////////////////
void ClearVideoStreamConfigCapsList(CWnd&Wnd)
{
	VIDEO_STREAM_CONFIG_CAPS **ppVScc = (VIDEO_STREAM_CONFIG_CAPS **)GetWindowLong(Wnd.m_hWnd, GWL_USERDATA);
	AM_MEDIA_TYPE *pmt;
	CComboBox cCombo;
	if (Wnd.IsKindOf(RUNTIME_CLASS(CComboBox)))
	{
	...
	for (i=0; i<nCount; i++)
	{
		pmt = (AM_MEDIA_TYPE*)cCombo.GetItemData(i);
		if (pmt)
		{
			DeleteMediaType(pmt);
		}
		SAFE_COTASKMEMFREE(ppVScc[i]);
	}
	SAFE_COTASKMEMFREE(ppVScc);
	cCombo.ResetContent();
	cCombo.m_hWnd = NULL;
	SetWindowLong(Wnd.m_hWnd, GWL_USERDATA, (long)0);
}
//////////////////////////////////////////////////////////////////////////
HRESULT AddAudioStreamConfigCapsToList(IBaseFilter *pFilter, int nPin, CWnd &Wnd)
{
	AM_MEDIA_TYPE *pmt=NULL;
	int i;
	HRESULT          hr        = NOERROR;
	...
	ClearAudioStreamConfigCapsList(Wnd);
	CComboBox cCombo;
	if (Wnd.IsKindOf(RUNTIME_CLASS(CComboBox)))
	{
		cCombo.m_hWnd = Wnd.m_hWnd;
	...
	GetPin(pFilter, PINDIR_OUTPUT, nPin, &pPinOut);
	if (FAILED(hr))
	{
		return hr;
	}
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			SetWindowLong(cCombo.m_hWnd, GWL_USERDATA, (long)pASCC);
			WAVEFORMATEX *pWFex;
			...
			for (i=0; i<iCount; i++)
			{
				...
				str.Format(_T("%s (%d Bytes/Sec, %d Hz)"), sSubtype, pWFex->nAvgBytesPerSec, pWFex->nSamplesPerSec);
				cCombo.AddString(str);
				cCombo.SetItemData(i, (ULONG)pmt);
			}
			RELEASE_OBJECT(pCfg);
...
//////////////////////////////////////////////////////////////////////////
void ClearAudioStreamConfigCapsList(CWnd&Wnd)
{
	AUDIO_STREAM_CONFIG_CAPS *pASCC = (AUDIO_STREAM_CONFIG_CAPS *)GetWindowLong(Wnd.m_hWnd, GWL_USERDATA);;
	SAFE_COTASKMEMFREE(pASCC);
	SetWindowLong(Wnd.m_hWnd, GWL_USERDATA, (long)0);
