...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegistryPath)
{
    ...
    // Create our device.
    Status = GpdCreateDevice(GPD_DEVICE_NAME, GPD_TYPE, DriverObject, &DeviceObject);
...
******************************************************************************************/
NTSTATUS GpdCreateDevice(IN PWSTR PrototypeName, IN DEVICE_TYPE DeviceType, IN PDRIVER_OBJECT DriverObject, OUT PDEVICE_OBJECT *ppDevObj)
{
        ...
        case IRP_MJ_DEVICE_CONTROL:
            ...
            switch (pIrpStack->Parameters.DeviceIoControl.IoControlCode)
            {
				...
				case IOCTL_GPD_READ_PORT_ULONG:
					Status = GpdIoctlReadPort(pLDI,	pIrp, pIrpStack, pIrpStack->Parameters.DeviceIoControl.IoControlCode);
	                break;
...
******************************************************************************************/
NTSTATUS GpdIoctlReadPort(IN PLOCAL_DEVICE_INFO pLDI, IN PIRP pIrp, IN PIO_STACK_LOCATION IrpStack,	IN ULONG IoctlCode)
{
    ...
    if (pLDI->PortMemoryType == 1)
    {
        ...
        {
			case IOCTL_GPD_READ_PORT_UCHAR:
				*(PUCHAR)pIOBuffer = READ_PORT_UCHAR((PUCHAR)((ULONG)pLDI->PortBase + nPort));
				break;
			case IOCTL_GPD_READ_PORT_USHORT:
				*(PUSHORT)pIOBuffer = READ_PORT_USHORT((PUSHORT)((ULONG)pLDI->PortBase + nPort));
				break;
			case IOCTL_GPD_READ_PORT_ULONG:
				*(PULONG)pIOBuffer = READ_PORT_ULONG((PULONG)((ULONG)pLDI->PortBase + nPort));
				break;
        }
    }
	else
	{
        ...
        {
			case IOCTL_GPD_READ_PORT_UCHAR:
				*(PUCHAR)pIOBuffer = READ_REGISTER_UCHAR((PUCHAR)((ULONG)pLDI->PortBase + nPort));
				break;
			case IOCTL_GPD_READ_PORT_USHORT:
				*(PUSHORT)pIOBuffer = READ_REGISTER_USHORT((PUSHORT)((ULONG)pLDI->PortBase + nPort));
				break;
			case IOCTL_GPD_READ_PORT_ULONG:
				*(PULONG)pIOBuffer = READ_REGISTER_ULONG((PULONG)((ULONG)pLDI->PortBase + nPort));
				break;
		}
    }
...
Return Value:
    ...
    STATUS_ACCESS_VIOLATION  -- An illegal port number was given.
******************************************************************************************/
NTSTATUS GpdIoctlWritePort(IN PLOCAL_DEVICE_INFO pLDI, IN PIRP pIrp, IN PIO_STACK_LOCATION IrpStack, IN ULONG IoctlCode)
{
                                // NOTE:  Use METHOD_BUFFERED ioctls.
    ...
    if (pLDI->PortMemoryType == 1)
    {
        ...
        {
			case IOCTL_GPD_WRITE_PORT_UCHAR:
				WRITE_PORT_UCHAR((PUCHAR)((ULONG)pLDI->PortBase + nPort), *(PUCHAR)pIOBuffer);
				break;
			case IOCTL_GPD_WRITE_PORT_USHORT:
				WRITE_PORT_USHORT((PUSHORT)((ULONG)pLDI->PortBase + nPort), *(PUSHORT)pIOBuffer);
				break;
			case IOCTL_GPD_WRITE_PORT_ULONG:
				WRITE_PORT_ULONG((PULONG)((ULONG)pLDI->PortBase + nPort), *(PULONG)pIOBuffer);
				break;
        }
    }
	else
	{
        ...
        {
			case IOCTL_GPD_WRITE_PORT_UCHAR:
				WRITE_REGISTER_UCHAR((PUCHAR)((ULONG)pLDI->PortBase + nPort),*(PUCHAR)pIOBuffer);
				break;
			case IOCTL_GPD_WRITE_PORT_USHORT:
				WRITE_REGISTER_USHORT((PUSHORT)((ULONG)pLDI->PortBase + nPort),	*(PUSHORT)pIOBuffer );
				break;
			case IOCTL_GPD_WRITE_PORT_ULONG:
				WRITE_REGISTER_ULONG((PULONG)((ULONG)pLDI->PortBase + nPort),*(PULONG)pIOBuffer );
				break;
        }
    }
