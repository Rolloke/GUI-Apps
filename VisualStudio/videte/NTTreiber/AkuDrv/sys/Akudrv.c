...
/*
    ...
    BOOLEAN				ResourceConflict;			// This is set true if our I/O ports  conflict with another driver
    ...
    // this driver.
    {
        ...
        if (paramPath.Buffer != NULL)
        {
            ...
            if (!NT_SUCCESS(RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE | RTL_REGISTRY_OPTIONAL, paramPath.Buffer, &paramTable[0], NULL, NULL)))
            {
    ...
    ResourceList.List[0].PartialResourceList.PartialDescriptors[0].u.Port.Length	= PortCount;
...
*/
    PortAddress.LowPart  = BASEPORT_LOW;
    PortAddress.HighPart = 0;
    PortCount = BASEPORT_COUNT;
...
******************************************************************************************/
NTSTATUS AkuCreateDevice(IN PWSTR PrototypeName, IN DEVICE_TYPE DeviceType, IN PDRIVER_OBJECT DriverObject, OUT PDEVICE_OBJECT *ppDevObj)
{
...
NTSTATUS AkuDispatch(IN PDEVICE_OBJECT pDO, IN PIRP pIrp)
{
    ...
    switch (pIrpStack->MajorFunction)
    {
        ...
        case IRP_MJ_DEVICE_CONTROL:
            ...
            {
				case IOCTL_AKU_READ_PORT_UCHAR:
				case IOCTL_AKU_READ_PORT_USHORT:
				case IOCTL_AKU_READ_PORT_ULONG:
					Status = AkuIoctlReadPort(pLDI,	pIrp, pIrpStack, pIrpStack->Parameters.DeviceIoControl.IoControlCode);
	                break;

				case IOCTL_AKU_WRITE_PORT_UCHAR:
				case IOCTL_AKU_WRITE_PORT_USHORT:
				case IOCTL_AKU_WRITE_PORT_ULONG:
					Status = AkuIoctlWritePort(pLDI, pIrp, pIrpStack, pIrpStack->Parameters.DeviceIoControl.IoControlCode);
					break;
            }
            break;
    }
...
******************************************************************************************/
NTSTATUS AkuIoctlReadPort(IN PLOCAL_DEVICE_INFO pLDI, IN PIRP pIrp, IN PIO_STACK_LOCATION IrpStack,	IN ULONG IoctlCode)
{
...
*/
	if ((nPort < BASEPORT_LOW) || nPort >= (BASEPORT_LOW + BASEPORT_COUNT))
        return STATUS_ACCESS_VIOLATION;   // It was not legal.
    ...
    if (pLDI->PortMemoryType == 1)
    {
        ...
        {
			case IOCTL_AKU_READ_PORT_UCHAR:
				*(PUCHAR)pIOBuffer = READ_PORT_UCHAR((PUCHAR)(nPort));
				break;
			case IOCTL_AKU_READ_PORT_USHORT:
				*(PUSHORT)pIOBuffer = READ_PORT_USHORT((PUSHORT)(nPort));
				break;
			case IOCTL_AKU_READ_PORT_ULONG:
				*(PULONG)pIOBuffer = READ_PORT_ULONG((PULONG)(nPort));
				break;
        }
    }
	else
	{
        ...
        {
			case IOCTL_AKU_READ_PORT_UCHAR:
				*(PUCHAR)pIOBuffer = READ_REGISTER_UCHAR((PUCHAR)(nPort));
				break;
			case IOCTL_AKU_READ_PORT_USHORT:
				*(PUSHORT)pIOBuffer = READ_REGISTER_USHORT((PUSHORT)(nPort));
				break;
			case IOCTL_AKU_READ_PORT_ULONG:
				*(PULONG)pIOBuffer = READ_REGISTER_ULONG((PULONG)(nPort));
				break;
		}
    }
...
Return Value:
    ...
    STATUS_ACCESS_VIOLATION  -- An illegal port number was given.
******************************************************************************************/
NTSTATUS AkuIoctlWritePort(IN PLOCAL_DEVICE_INFO pLDI, IN PIRP pIrp, IN PIO_STACK_LOCATION IrpStack, IN ULONG IoctlCode)
{
                                // NOTE:  Use METHOD_BUFFERED ioctls.
    PULONG	pIOBuffer;          // Pointer to transfer buffer (treated as array of longs)
    ULONG	InBufferSize ;      // Amount of data avail. from caller.
    ULONG	OutBufferSize ;     // Max data that caller can accept.
...
*/
    ...
    if (pLDI->PortMemoryType == 1)
    {
        ...
        {
			case IOCTL_AKU_WRITE_PORT_UCHAR:
				WRITE_PORT_UCHAR((PUCHAR)(nPort),   *(PUCHAR)pIOBuffer);
				break;
			case IOCTL_AKU_WRITE_PORT_USHORT:
				WRITE_PORT_USHORT((PUSHORT)(nPort), *(PUSHORT)pIOBuffer);
				break;
			case IOCTL_AKU_WRITE_PORT_ULONG:
	...
	else
	{
        ...
        {
			case IOCTL_AKU_WRITE_PORT_UCHAR:
				WRITE_REGISTER_UCHAR((PUCHAR)(nPort),   *(PUCHAR)pIOBuffer);
				break;
			case IOCTL_AKU_WRITE_PORT_USHORT:
				WRITE_REGISTER_USHORT((PUSHORT)(nPort),	*(PUSHORT)pIOBuffer );
				break;
			case IOCTL_AKU_WRITE_PORT_ULONG:
				WRITE_REGISTER_ULONG((PULONG)(nPort),   *(PULONG)pIOBuffer );
				break;
        }
    }
