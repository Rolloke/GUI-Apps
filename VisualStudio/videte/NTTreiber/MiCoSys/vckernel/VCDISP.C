...
--*/
{
	...
	case IOCTL_VIDC_OVERLAY_MODE:
	    /*
             * MICO: This is the WatchDog Function
	     */
		...
		pDevInfo->Callback.GetOverlayModeFunc(pDevInfo, pConfig);
		pIrp->IoStatus.Information = 0;
	...
	case IOCTL_VIDC_SET_KEY_RGB:
		...
		if (pDevInfo->Callback.SetKeyRGBFunc(pDevInfo)) {
		    Status = STATUS_SUCCESS;
		} else {
		    Status = STATUS_DEVICE_CONFIGURATION_ERROR;
		}
	...
	case IOCTL_VIDC_OVERLAY_RECTS:
	    /*
	     * MICO: This is the ALARM Function
 	     */
		...
		pConfig = (PCONFIG_INFO)pIrp->AssociatedIrp.SystemBuffer;
	        ...
	        pIrp->IoStatus.Information =
                      pDevInfo->Callback.SetOverlayRectsFunc(pDevInfo, pConfig);
                if (pIrp->IoStatus.Information == 0xffff)
		   Status = STATUS_INVALID_DEVICE_REQUEST;
		else
		   Status = STATUS_SUCCESS;
	    } else {
	...
	case IOCTL_VIDC_OVERLAY_OFFSET:
	    /*
	     * MICO: This is the RELAIS Function
	     */
		...
		pIrp->IoStatus.Information =
                      pDevInfo->Callback.SetOverlayOffsetFunc(pDevInfo, pConfig);
                if (pIrp->IoStatus.Information == 0xffff)
		   Status = STATUS_INVALID_DEVICE_REQUEST;
		else
		   Status = STATUS_SUCCESS;
