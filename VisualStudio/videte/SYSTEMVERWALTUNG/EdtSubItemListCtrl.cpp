...
BOOL CEdtSubItemListCtrl::SelectSubItem(int nItem, int nSubItem)
{
		...
		|| m_Selected.Cell.wSubItem != nSubItem)
	{
			...
			)
		{
			...
			uListItem Old = m_Selected;
			m_Selected.Cell.wItem    = (WORD)nItem;
			m_Selected.Cell.wSubItem = (WORD)nSubItem;
			CRect rc;
			if (m_Selected.Cell.wItem != Old.Cell.wItem)
			{
				CListCtrl::GetSubItemRect(m_Selected.Cell.wItem, 0, LVIR_BOUNDS, rc);
				InvalidateRect(&rc, FALSE);
				CListCtrl::GetSubItemRect(Old.Cell.wItem, 0, LVIR_BOUNDS, rc);
				InvalidateRect(&rc, FALSE);
			}
			else
			{
				GetSubItemRect(m_Selected.Cell.wItem, m_Selected.Cell.wSubItem, rc);
				InvalidateRect(&rc, FALSE);
				GetSubItemRect(Old.Cell.wItem, Old.Cell.wSubItem, rc);
				InvalidateRect(&rc, FALSE);
			}
			NMITEMACTIVATE nmia;
			ZERO_INIT(nmia);
