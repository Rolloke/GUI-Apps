...
void CStatusList::SetRadioItemFromlParam(LPARAM lParam)
{
	...
	{
		BOOL bFound = FALSE;
		for (int i=0 ; !bFound && i<GetItemCount() ; i++)
		{
...
void CStatusList::OnSpace() 
{
	...
	((CSVPage*)GetParent())->OnStatusChangedStatusList(GetDlgCtrlID(), nHitItem);
...
void CStatusList::OnItemchanged(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if ((pNMListView->iItem!=m_RadioIndex) && (m_nStates==0))
	{
		...
		((CSVPage*)GetParent())->OnItemChangedStatusList(this, pNMListView->iItem);
	}
	else if (m_nStates>0)
	{
		((CSVPage*)GetParent())->OnItemChangedStatusList(this, pNMListView->iItem);
