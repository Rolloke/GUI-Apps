...
{
	TRACE(_T("ConnectThreadProc Start\n"));

	CServerAlarmlist* pServer = (CServerAlarmlist*)pParam;

	XTIB::SetThreadName(_T("CServerAlarmlist::ConnectThreadProc"));
	...
	if (!pServer->IsDatabaseConnected())
	{
		...
		WK_TRACE(_T("FetchDatabaseConnection\n"));
		CIPCFetch fetch;
	...
	TRACE(_T("ConnectThreadProc Ende\n"));
...
void CServerAlarmlist::ConnectDatabase()
{
	...
	if (m_pDatabaseResult)
	{
		...
		{
			WK_TRACE(_T("connect database OK\n"));
			WK_DELETE(m_pDatabase);
...
void CServerAlarmlist::OnIdle()
{
	...
	if (   m_pConnectThread 
		&& !m_bConnectThreadRunning)
	{
		TRACE(_T("deleting connection thread\n"));
