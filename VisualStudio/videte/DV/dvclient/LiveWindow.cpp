...
#define PL_PERCENT  3
static const int g_nQualities[g_nLevelArraySize][4] = 
{// Active, MD,  no MD,  %   Level
	{   25,  5,     5, 100}, //  0	// positive value mean frames per second
	{   25,  5,    -5,  90}, //  1	// negative values mean 100 ms timeout value
	{    5,  5,    -5,  80}, //  2	// factors. the negative sign is used as indicator
	{    5, -5,    -5,  70}, //  3	// e.g. -5: 100 * 5 = 500 ms timeout
	{   -5, -5,    -5,  60}, //  4	//  between picture requests
	{   -5, -5,   -10,  50}, //  5
	{   -5,-10,   -10,  40}, //  6
	{  -10,-10,   -10,  30}, //  7
	{  -10,-10,   -20,  20}, //  8
...
//	TRACE(_T("%s"), or.GetName());
	...
	if (theApp.m_bUseMMtimer == TIMER_EVENT)
	{
...
int CLiveWindow::GetPerformancePercent()
{
	...
	ASSERT(nPerfLevel < g_nLevelArraySize);
	return g_nQualities[nPerfLevel][PL_PERCENT];
}
/////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////
void CLiveWindow::SetActive(BOOL bActive)
{
	if (bActive && m_bIsActive != bActive)
	{
		theApp.GetMainFrame()->PostMessage(WM_USER,MAKELONG(WPARAM_REQUEST,0),RM_PERFORMANCE);
	}
	CDisplayWindow::SetActive(bActive);
}
/////////////////////////////////////////////////////////////////////////////
...
//				}
	...
	else if (IsYUV())	// uncompressed local
	{
		CIPCOutputDVClient* pOutput = m_pServer->GetOutput();
		int iIFrameInt		= 1;
		bRet = 0;
...
/////////////////////////////////////////////////////////////////////////////
BOOL CLiveWindow::PictureData(const CIPCPictureRecord &pict,DWORD dwX, DWORD dwY)
{
	// TRACE(_T("Coordinates:%d, %d\n"), dwX, dwY);
	BOOL bRet = CDisplayWindow::PictureData(pict, dwX, dwY);
	//TRACE(_T("confirm %s %s\n"),GetName(),pict.GetTimeStamp().GetTime());
	if (bRet)
	{
		...
		else if(pict.GetCompressionType() == COMPRESSION_MPEG4)
		{
			...
			if (pict.GetPictureType() == SPT_FULL_PICTURE || pict.GetPictureType() == SPT_GOP_PICTURE)
			{
				BOOL bSetGOP = FALSE;
				if (nNumPics > 1)
				{
...
//						TRACE(_T("Adding queue picture %d\n"), m_GOPs.GetSize());
					}
					else
					{
						m_pGOP = new CIPCPictureRecord(pict);
						m_nCurrentPicture = 0;
					}
					acs.Unlock();
				...
				if (!bSetGOP)
				{
					...
					CAutoCritSec acs(&m_csPictureRecord);
					m_pMpeg->DecodeMpeg4FromMemory((BYTE*)pict.GetData(), pict.GetDataLength());
					acs.Unlock();
					ShowVideo();
					if (m_pStatistics)
					{
			...
			{
				TRACE(_T("MPEG-4 frame dropped %s\n"),GetName());
			}
		}
		Sleep(0);
	}
...
int CLiveWindow::CalculateFrameStep(int nTimeStep)
{
	...
	//				 30, 25, 12, 5 fps
	int nValues[] = {33, 40, 83, 200}; // ms
	int i, nSel = 0, nDiff, nBest = 1000;
	for (i=0; i<4; i++)
	{
		nDiff = abs(nTimeStep - nValues[i]);
		if (nDiff < nBest)
		{
			nBest = nDiff;
			nSel = i;
...
void CLiveWindow::SetGOPTimer( UINT uElapse)
{
	if (theApp.m_bUseMMtimer)
	{
		...
		if (theApp.m_bUseMMtimer == TIMER_PROC)
		{
			m_uGOPTimer = ::timeSetEvent(uElapse, gm_wTimerRes, GOPTimer, (DWORD)this, TIME_PERIODIC);
		}
		else if (theApp.m_bUseMMtimer == TIMER_EVENT)
		{
...
UINT CLiveWindow::GOPThread(LPVOID lpParam)
{
	...
	do
	{
		dwWait = WaitForSingleObject(pThis->m_hGOPTimerEvent, 1000);
		if (dwWait == WAIT_OBJECT_0 && pThis->m_hWnd && pThis->m_uGOPTimer)
		{
			pThis->OnTimer(pThis->m_uGOPTimer);
		}
	}
	while (pThis->m_hWnd != NULL);
...
{
	CAutoCritSec acs(&m_csPictureRecord);
	if (nIDEvent == m_uGOPTimer && m_pMpeg != NULL)
	{
		if (m_nCurrentPicture == -1)
		{
//			TRACE(_T("No Pictures\n"));
			PostMessage(WM_USER, ID_REQUEST, -1);
			return;
		}
		int nNumPictures = 0;
		if (m_pGOP)
		{
			...
			if (theApp.IsReceiver())	// größeren puffer zulassen
			{
				nNumGOPS -= 1;
			}
			nNumPictures = m_pGOP->GetNumberOfPictures();
			int nMax = max(1, 5 - nNumGOPS);
			PostMessage(WM_USER, ID_REQUEST, nMax);
			if (nNumGOPS > 1)
			{
				nNumPictures = nNumPictures - nNumGOPS + 1;
				TRACE(_T("Reducing no of pics in GOP %d of %d in %s\n"), 
					nNumPictures, m_pGOP->GetNumberOfPictures(), GetName());
			}
			else
			{
		...
		if (m_pGOP)
		{
			BOOL bDecoded = m_pMpeg->DecodeMpeg4FromMemory((BYTE*)m_pGOP->GetData(m_nCurrentPicture),
												m_pGOP->GetDataLength(m_nCurrentPicture));
			if (bDecoded)
			{
				ShowVideo();
				if (m_pStatistics)
				{
					m_pStatistics->AddValue(m_pGOP->GetDataLength(m_nCurrentPicture));
				}
			}
...
void CLiveWindow::GetNextPictureOfGOP()
{
	...
	if (m_GOPs.GetSize())
	{
		m_pGOP = m_GOPs.GetAt(0);
		m_GOPs.RemoveAt(0);
		m_nCurrentPicture = 0;
	}
	else
	{
//		TRACE(_T("No more Pictures\n"));
		m_nCurrentPicture = -1;
	}
	PostMessage(WM_USER, ID_REQUEST, -1);
}
...
BOOL CLiveWindow::GetUnitFPS()
{
	if (m_pServer && m_pServer->IsOutputConnected())
	{
		...
		if (pDVOC)
		{
			CSecID id (m_orOutput.GetID().GetGroupID(), SECID_NO_SUBID);
			pDVOC->DoRequestGetValue(id, CSD_FPS,0);
			return TRUE; // request fired
		}
	}
	return FALSE;
}
...
LRESULT CLiveWindow::OnUser(WPARAM wParam, LPARAM lParam)
{
	switch (wParam)
	{
		...
		default: return CDisplayWindow::OnUser(wParam, lParam);
	}
	return 0;
}
/////////////////////////////////////////////////////////////////////////////
...
void CLiveWindow::OnDraw(CDC* pDC)
{
	...
	// Bei UVV-Kassen soll kein Fadenkreuz zu sehen sein.
	if (!IsMD())
		SetMDValues(0,0, TRUE);
...
void CLiveWindow::OnCancel()
{
	if (m_pBtnCancel)
	{
		...
		GetParent()->PostMessage(WM_USER,MAKELONG(WPARAM_REQUEST,0),RM_PERFORMANCE);
		m_iRequests = 0;
