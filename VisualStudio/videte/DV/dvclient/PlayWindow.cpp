...
/////////////////////////////////////////////////////////////////////////////
BOOL CPlayWindow::SetMDValues(WORD wX, WORD wY, BOOL bForce)
{
	if (wX>0)
	{
		m_bCanMDTrack = TRUE;
	}
	return CDisplayWindow::SetMDValues(wX,wY, bForce);
}
/////////////////////////////////////////////////////////////////////////////
		...
		*/
		...
		if(m_bCanRectSearch && !theApp.GetMainFrame()->IsInBackup())
		{
			...
			if (pFX && pFY)
			{
				...
				// Bei MPEG4 enthält immer nur das I-Frame MD-Koordinaten. Bei den P-Frames soll
				// das Fadenkreuz weiterhin an der Stelle zu sehen sein, die es beim I-Frame hatte.
				if ((pict.GetCompressionType() != COMPRESSION_MPEG4) || 
					((pict.GetCompressionType() == COMPRESSION_MPEG4) && (pict.GetPictureType() == SPT_FULL_PICTURE)))
				{
					RemoveAllMDValues();
				}
						   ...
						   int nRecCount /*=0*/, DWORD dwCam /*= (DWORD)(-1)*/)
{
	if(theApp.TKR_Trace())
	{
		TRACE(_T("#### Navigate %s %08lx Seq: %d  RecNr: %d\n"),
			NameOfEnum(m_PlayStatus),GetCurrentThreadId(), wSequenceNr, dwRecordNr);
	}	
	BOOL bRet = FALSE;
...
BOOL CPlayWindow::IndicateQueryResult(WORD wSequenceNr, DWORD dwRecordNr)
{
												...
												//auswerten, ob gerade eine Rechtecksuche läuft
	{
		if (IsWindowVisible())
		{
			//bei der Rechtecksuche nur das gefundene Bild anzeigen, wenn der RechteckPlayStatus
			//nicht auf Stop ist
			if(m_RectPlayStatus != PS_RECT_STOP)
			{	
			...
			{
				TRACE(_T("unexpected rectangle query result %d,%d\n"),wSequenceNr,dwRecordNr);
				return FALSE;
			}
		}
		else
