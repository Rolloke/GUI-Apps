...
CDisplayWindow::CDisplayWindow(CMainFrame* pParent, CServer* pServer)
{
	...
	m_resCurrent = RESOLUTION_NONE;

	CWK_Profile prof;
	m_bAllowDeinterlacing = prof.GetInt(_T("DV\\DVClient"), _T("AllowDeinterlacing"), 0);
	prof.WriteInt(_T("DV\\DVClient"), _T("AllowDeinterlacing"), m_bAllowDeinterlacing);
...
/////////////////////////////////////////////////////////////////////////////
BOOL CDisplayWindow::PictureData(const CIPCPictureRecord &pict,DWORD dwX, DWORD dwY)
{
	if (m_hWnd == NULL)
	{
		ASSERT(FALSE);
	...
	if (m_pPictureRecord)
	{
		if (  (pict.GetBlockNr() == 0) 
			||(pict.GetBlockNr() != m_pPictureRecord->GetBlockNr())
			)
		{
	...
	if (   COMPRESSION_JPEG == pict.GetCompressionType()
		|| COMPRESSION_YUV_422 == pict.GetCompressionType()
		|| COMPRESSION_RGB_24 == pict.GetCompressionType())
	{
		CreateJpegDecoder();
	}
	if (COMPRESSION_MPEG4 == pict.GetCompressionType())
	{
		CreateMpegDecoder();
	}
...
/////////////////////////////////////////////////////////////////////////////
BOOL CDisplayWindow::SetMDValues(WORD wX, WORD wY, BOOL bForce/*=FALSE*/)
{
	m_csPictureRecord.Lock();
	...
	else if (bForce || ((dwTickCount - m_dwLastMD_TC) > MOTION_DETECTION_TIMEOUT))
	{
		m_bMD = FALSE;
		m_wMD_X = 0;
		m_wMD_Y = 0;
...
void CDisplayWindow::OnChangeVideo() 
{
	...
	if (m_pPictureRecord)
	{
		...
		switch (m_pPictureRecord->GetCompressionType())
		{
			...
			if (m_pJpeg)
			{
				if (m_pPictureRecord->GetResolution() == RESOLUTION_4CIF)
				{
					CRect rect;
					GetClientRect(rect);
					if (rect.Height()>288)
					{
						// real deinterlace
						m_pJpeg->SetDeInterlace(1);
					}
					else
					{
						// simple double even half field
						m_pJpeg->SetDeInterlace(2);
					}
				}
				else
				{
					m_pJpeg->SetDeInterlace(0);
				}
...
void CDisplayWindow::OnDraw(CDC* pDC)
{
	if (m_pPictureRecord)
	{
		...
		if (m_pPictureRecord->GetResolution() != m_resCurrent)
		{
			// Bei einem Auflˆsungswechsel wird ein evtl. vorhandener Zoom deaktiviert, da ansonsten
			// evtl. auf nich vorhandene Bildteile zugegriffen wird.
			m_rcZoom = CRect(0,0,0,0);
			m_resCurrent = m_pPictureRecord->GetResolution();
		}
