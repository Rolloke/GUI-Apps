...
public:
	...
	//wird im "CIPCDatabaseDVClient::OnConfirmRecordNr" aufgerufen
	...
	void				SetHandlePlayWindow(HWND hPlayWindow);
	...
	//get timestamp from a picture
	CSystemTime			GetRealTimePictureDateTime();
	...
	DWORD	GetLastShowTimesCount();
	void	SetShowTime(DWORD dwTick); //Dauer für Anzeigen des letzten Bildes 
	DWORD	GetShowTime();
...
private:
	...
	const CIPCFields*			m_pRealTimeFields;


	HWND						m_hPlayWindow;					//handle to playwindow
