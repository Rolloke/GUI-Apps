...
BOOL CTape::CreateDbf()
{
	if ((m_pData4==NULL) && (m_dwNumRecords == 0))
	{
		...
		else
		{
			...
			errorSet();
			WK_DELETE(m_pData4);
			theApp.HandleFileException(MAKELONG(0, (short)ret), sDbf);
			theApp.OnError();
		}
	}
	return IsOpenDbf();
...
BOOL CTape::CreateDat()
{
	...
	if (!m_pCollection->IsAlarmList())
	{
		...
		if (m_fImages.Open(sDat, CFile::modeCreate|CFile::modeReadWrite, &cfe))
		{
			...
			if (theApp.GetNoFrag())
			{
				...
				CATCH(CFileException, pCfe)
				{
					...
					theApp.HandleFileException(pCfe->m_lOsError, pCfe->m_strFileName);
				}
				END_CATCH
			}
		}
		else
		{
			...
			bRet = FALSE;
			theApp.HandleFileException(cfe.m_lOsError, cfe.m_strFileName);
		}
	}
	return bRet;
}
...
BOOL CTape::CreateDir()
{
	...
	if (!DoesFileExist(sDir))
	{
		...
		else
		{
			...
			WK_TRACE_ERROR(_T("cannot create directory %s, %s"), sDir, GetLastErrorString(dwLastError));
			bRet = FALSE;
			theApp.HandleFileException(dwLastError, sDir);
		}
	}
	return bRet;
}
...
BOOL CTape::Create()
{
	CAutoCritSec acs(&m_cs, _T("S1"), DVS_TIMEOUT);
...
BOOL CTape::AreFieldsEqual()
{
	...
	CAutoCritSec acs(&m_cs, _T("S17"), DVS_TIMEOUT);
...
BOOL CTape::OpenDbf()
{
	CAutoCritSec acs(&m_cs, _T("S2"), DVS_TIMEOUT);
...
//		CString sDbf;
		...
		else
		{
			...
			errorSet();
			WK_DELETE(m_pData4);
			theApp.HandleFileException(MAKELONG(0, r), sDbf);
			
			if (!DoesFileExist(sDbf))
			{
				WK_TRACE_ERROR(_T("file doesn't exist <%s>\n"), LPCTSTR(sDbf));
				theApp.HandleFileException(ERROR_FILE_NOT_FOUND, sDbf);
			}
			else if (!IsBackup())
			{
				if (    r==e4data 
...
BOOL CTape::OpenDat()
{
	if (!m_pCollection->IsAlarmList())
	{
		CAutoCritSec csTape(&m_cs, _T("S3"), DVS_TIMEOUT);
		...
		if (!IsOpenDat())
		{
			...
			else
			{
				...
				theApp.HandleFileException(cfe.m_lOsError, sDat);
			}
		}
		csTape.Unlock();
	}
...
BOOL CTape::CloseDbf(BOOL bForce /*= FALSE*/)
{
	...
	CAutoCritSec acs(&m_cs, _T("S4"), DVS_TIMEOUT);
	...
	if (IsOpenDbf())
	{
		...
		if ((m_iOpenDbf==0) || bForce)
		{
			...
			else
			{
				...
				errorSet();
				bRet = FALSE;
				theApp.HandleFileException(MAKELONG(0, (WORD)ret), GetFileName(szDbf));
			}
		}
		else
		{
...
BOOL CTape::Delete()
{
	...
	if (!IsBackup())
	{
		CAutoCritSec csTape(&m_cs, _T("S6"), DVS_TIMEOUT);
...
//				TRACE(_T("DeleteFile(%s)\n"),GetFileName(szDat));
				...
				{
					DWORD dwLastError = GetLastError();
					WK_TRACE_ERROR(_T("cannot delete %s, %s\n"),GetFileName(szDat), GetLastErrorString(dwLastError));
					theApp.HandleFileException(dwLastError, GetFileName(szDat));
				}
			}
		}
		else
		{
			DWORD dwLastError = GetLastError();
			WK_TRACE_ERROR(_T("cannot delete %s, %s\n"),GetFileName(szDbf), GetLastErrorString(dwLastError));
			theApp.HandleFileException(dwLastError, GetFileName(szDbf));
		}
...
{
	CAutoCritSec csTape(&m_cs, _T("S7"), DVS_TIMEOUT);
			...
			&& ret == r4success)
		{
			...
			CATCH (CFileException, cfe)
			{
				...
				WK_TRACE(_T("cannot write picture %s %s\n"),
					GetFileName(szDat), GetLastErrorString(cfe->m_lOsError));
				theApp.HandleFileException(cfe->m_lOsError, cfe->m_strFileName);
			}
			END_CATCH
		}	// write data
...
BOOL CTape::Move(CTape& source)
{
	...
	CAutoCritSec csSource(&source.m_cs, _T("S12"), DVS_TIMEOUT);
	CAutoCritSec csTape(&m_cs, _T("S13"), DVS_TIMEOUT);
	...
	if (!MoveFile(sf,df))
	{
		bRet = FALSE;
		DWORD dwLastError = GetLastError();
		WK_TRACE(_T("cannot move %s to %s because %s\n"), sf, df, GetLastErrorString(dwLastError));
		theApp.HandleFileException(dwLastError, df);
	}
