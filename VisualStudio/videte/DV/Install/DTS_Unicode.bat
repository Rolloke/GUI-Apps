...
$DEST_DIR="d:\\install\\dts\\unicode";
...
# oem DTS is Original
print "$WISE_CMD /c \"$WISE_DIR\\dts\\dts_transmitter_Unicode.wse\" \n";
...
{ 
	...
	print "v:\\bin\\tools\\MakeUpdateDisk.exe $DEST_DIR $oemNumber 599\n";
	`v:\\bin\\tools\\MakeUpdateDisk.exe $DEST_DIR $oemNumber 599`;
...
#	`copy $DEST_DIR\\Update\\Disk1\\update $DEST_DIR\\$oemName\\update`;
	`xcopy $DEST_DIR\\Update\\*.* $DEST_DIR\\$oemName\\ /s /y`;
}
