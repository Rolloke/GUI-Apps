...
/*		WK_TRACE(_T("QUERY%d result Nr= %d A=%d,S=%d,R=%d\n"),
		...
		//m_dwMaxQueryResults übermittelt 2 Flags, ist im Idip eingebaut
		//LO=1: MaxQueryResults, HI=1: SpecialQueryCancel
		WORD wMaxQueryResults = HIWORD(m_dwMaxQueryResults);
	
		if (   wMaxQueryResults == 0
			|| (DWORD)m_Results.GetSize() < wMaxQueryResults)
		{
		...
		if (   wMaxQueryResults > 0
			&& m_dwSentQueryResults >= wMaxQueryResults
			)
		{
			Cancel();
...
//		|| m_dwSentQueryResults<m_dwMaxQueryResults)
		...
		|| m_dwSentQueryResults<wMaxQueryResults)
	{
		...
		{
			//WK_TRACE(_T("TKR ### DoIndicateQueryResult  ID: %d\n"), GetUserID());
