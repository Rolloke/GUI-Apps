// BackupThread.cpp: implementation of the CBackupThread class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "DVStorage.h"
#include "Mainfrm.h"
#include "BackupThread.h"

#ifdef _DEBUG
#undef THIS_FILE
static char THIS_FILE[]=__FILE__;
#define new DEBUG_NEW
#endif

//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////
CBackupThread::CBackupThread(CBackup* pBackup) : CWK_Thread(_T("BackupThread"),NULL)
{
	m_pBackup = pBackup;
	m_pBackup->SetBackupThread(this);
	m_bCancelled = FALSE;
	m_iCurrentTape = 0;
	m_bIsRunning = TRUE;
	m_bFinished = FALSE;
	m_dwTimeOut = 30*1000;
	m_dwID = 0;
	m_dwTimeOutHandled = 0;
	SetLastAction();
}
//////////////////////////////////////////////////////////////////////
void CBackupThread::SetLastAction()
{
	CAutoCritSec acs(&m_csTimeOut);
	m_dwLastAction = GetTickCount();
}
//////////////////////////////////////////////////////////////////////
CBackupThread::~CBackupThread()
{
	WK_DELETE(m_pBackup);
}
//////////////////////////////////////////////////////////////////////
BOOL CBackupThread::Run(LPVOID lpContext)
{
	BOOL bRet = FALSE;
	if (m_pBackup->GetFlags()==0)
	{
		// old fashioned backup
		bRet = RunCopy();
	}
	else
	{
		// new backup by time
		if (m_pBackup->GetFlags() == BBT_CALCULATE)
		{
			bRet = RunCalculate();
		}
		else if (m_pBackup->GetFlags() == BBT_CALCULATE_END)
		{
			bRet = RunCalculateEnd();
		}
		else if (m_pBackup->GetFlags() & BBT_EXECUTE)
		{
			bRet = RunCopy();
		}
	}

	if (!bRet)
	{
		CWnd* pWnd = AfxGetMainWnd();
		if (WK_IS_WINDOW(pWnd))
		{
			pWnd->PostMessage(WM_USER,WPARAM_BACKUP_FINISHED);
		}
		m_pBackup->SetBackupThread(NULL);
	}

	return bRet;
}
//////////////////////////////////////////////////////////////////////
BOOL CBackupThread::RunCopy()
{
	// return TRUE for next call of run
	SetLastAction();


	// always check if client still exits
	CClient* pClient = theApp.m_Clients.GetClient(m_pBackup->GetClientID());
	if (pClient==NULL)
	{
		WK_TRACE_ERROR(_T("backup stopped by disconnect or cancel\n"));
		return FALSE;
	}
	CIPCDatabaseStorage* pCIPCDatabase = pClient->GetCIPC();

	// does Client cancelled the backup ?
	if (m_bCancelled)
	{
		m_pBackup->CleanUp();
		pClient->GetCIPC()->DoConfirmCancelBackup(m_pBackup->GetUserData());
		return FALSE;
	}

	// are we ready ?
	if (m_iCurrentTape >= m_pBackup->GetNrOfIDs())
	{
		// write Sequence Map 
		m_pBackup->WriteSequenceMap();
		// end everything is ok
		// signal the end
		pCIPCDatabase->DoConfirmBackup(m_pBackup->GetUserData(),
									   m_pBackup->GetDestinationPath(),
									   MAKELONG(0,0));
		WK_TRACE(_T("backup finished\n"));
		m_bFinished = TRUE;
		return FALSE;
	}

	// Initialize, create directory and so on
	if (!m_pBackup->Initialized())
	{
		if (m_pBackup->Initialize())
		{
			SetLastAction();
			return TRUE;
		}
		else
		{
			WK_TRACE_ERROR(_T("backup initializing failed\n"));
			DoIndicateError(2,m_pBackup->GetUserData());
			return FALSE;
		}
	}

	// do the real copy
	m_dwID = m_pBackup->GetIDNr(m_iCurrentTape);
	WORD wArchiveNr   = HIWORD(m_dwID);
	WORD wSequenceNr = LOWORD(m_dwID);

	// increase index
	m_iCurrentTape++;

	// id wrong try the next
	if (m_dwID == 0)
	{
		WK_TRACE_ERROR(_T("backup Collection nr or Tape nr not found\n"));
		return TRUE;
	}

	// is the Archive still there
	CAutoCritSec acs(&theApp.m_Archives.m_cs);
	CCollection* pCollection = theApp.m_Archives.GetCollection(wArchiveNr);
	if (pCollection==NULL)
	{
		WK_TRACE_ERROR(_T("Collection for backup not found %d\n"),wArchiveNr);
		return TRUE;
	}

	// is the tape still there
	CTape* pTape = pCollection->GetTapeID(wSequenceNr);
	if (pTape==NULL)
	{
		WK_TRACE_ERROR(_T("Tape for backup not found %d\n"),wSequenceNr);
		SetLastAction();
		return TRUE;
	}

	// everything OK

	// calc the backup timeout limit
	DWORD dwSizeBytes = (DWORD)pTape->GetSizeBytes().GetInt64();
	// 150 kB/s = 150*1024B/1000ms = 153,6 Byte / ms
	// assume minimum speed of 50 Byte / ms
	// safety reserve of 60 s
	// now calc timeout in ms (!)
	m_dwTimeOut = dwSizeBytes / 50 + 60 * 1000;
	SetLastAction();

	// call the backup
	BOOL bRet = TRUE;

	if (m_pBackup->GetFlags() == 0)
	{
		bRet = pTape->Backup(*m_pBackup);
	}
	else
	{
		bRet = pTape->BackupByTime(*m_pBackup);
	}
	acs.Unlock();

	if (bRet)
	{
		pClient = theApp.m_Clients.GetClient(m_pBackup->GetClientID());
		if (pClient==NULL)
		{
			WK_TRACE_ERROR(_T("backup stopped by disconnect or cancel\n"));
			return FALSE;
		}
		pCIPCDatabase->DoConfirmBackup(m_pBackup->GetUserData(),
									   m_pBackup->GetDestinationPath(),
									   m_dwID);
		SetLastAction();
		// to give other threads more breath
		Sleep(50);
		// check whether it was an Collection to backup
		// and it's was the last Tape
		if (m_pBackup->WasLastTapeOfCollection(wArchiveNr,wSequenceNr))
		{
			WK_TRACE(_T("last sequence of %04hx backup complete\n"),wArchiveNr);
			pCIPCDatabase->DoConfirmBackup(m_pBackup->GetUserData(),
										   m_pBackup->GetDestinationPath(),
										   MAKELONG(0,wArchiveNr));
		}
	}
	else if (m_pBackup->GetFlags() == 0)
	{
		SetLastAction();
		DoIndicateError(3,m_dwID);
	}
	else
	{
		// OK nothing todo, because sequence is out of time 
	}

	return TRUE;
}
//////////////////////////////////////////////////////////////////////
BOOL CBackupThread::RunCalculate()
{
	CIPCInt64 iSum = 0;
	CIPCInt64 iOne = 0;
	DWORD dwTick = GetTickCount();
	CClient* pClient = NULL;

	for (int i=0;i<m_pBackup->GetNrOfIDs();i++)
	{
		DWORD dwID = m_pBackup->GetIDNr(i);
		WORD wArchiveNr   = HIWORD(dwID);
		WORD wSequenceNr = LOWORD(dwID);

		CAutoCritSec acs(&theApp.m_Archives.m_cs);
		CCollection* pCollection = theApp.m_Archives.GetCollection(wArchiveNr);
		
		if (   pCollection
			&& !pCollection->IsBackup())
		{
			if (wSequenceNr != 0)
			{
				CTape* pTape = pCollection->GetTapeID(wSequenceNr);
				if (   pTape
					&& !pTape->IsBackup())
				{
					iOne = pTape->GetSizeBytes(m_pBackup->GetStartTime(),
											   m_pBackup->GetEndTime());
					if (iOne>0)
					{
						pClient = theApp.m_Clients.GetClient(m_pBackup->GetClientID());
						if (   pClient
							&& pClient->IsConnected()
							&& !m_bCancelled)
						{
							pClient->GetCIPC()->DoConfirmBackupByTime((WORD)m_pBackup->GetUserData(),
								dwID,
								m_pBackup->GetStartTime(),
								m_pBackup->GetEndTime(),
								(WORD)BBT_CALCULATE,
								iOne);
						}
						iSum += iOne;
						if (   m_pBackup->GetMaximumInMB()>0
							&& iSum.GetInMB()>m_pBackup->GetMaximumInMB())
						{
							break;
						}
					}
				}
			}
			else
			{
				// calculate the entire archive
				iOne = pCollection->GetSizeBytes(m_pBackup->GetStartTime(),
												 m_pBackup->GetEndTime(),
												 iSum,
												 m_pBackup->GetMaximumInMB(),
												 m_bCancelled);
				if (iOne>0)
				{
					pClient = theApp.m_Clients.GetClient(m_pBackup->GetClientID());
					if (   pClient
						&& pClient->IsConnected())
					{
						pClient->GetCIPC()->DoConfirmBackupByTime((WORD)m_pBackup->GetUserData(),
																				 dwID,
																				m_pBackup->GetStartTime(),
																				m_pBackup->GetEndTime(),
																				 (WORD)BBT_CALCULATE,
																				 iOne);
					}
					if (   m_pBackup->GetMaximumInMB()>0
						&& iSum.GetInMB()>m_pBackup->GetMaximumInMB())
					{
						break;
					}
				}
			}
		}
		acs.Unlock();

		if (m_bCancelled)
		{
			pClient = theApp.m_Clients.GetClient(m_pBackup->GetClientID());
			if (   pClient
				&& pClient->IsConnected())
			{
				pClient->GetCIPC()->DoConfirmCancelBackup(m_pBackup->GetUserData());
			}
			break;
		}
	}

	pClient = theApp.m_Clients.GetClient(m_pBackup->GetClientID());
	if (   pClient
		&& pClient->IsConnected())
	{
		pClient->GetCIPC()->DoConfirmBackupByTime((WORD)m_pBackup->GetUserData(),
																 0,
																m_pBackup->GetStartTime(),
																m_pBackup->GetEndTime(),
																 (WORD)BBT_CALCULATE,
																 iSum);
	}
	WK_TRACE(_T("CALCULATE BACKUP in %d ms %s\n"),GetTimeSpan(dwTick),iSum.Format(TRUE));
	// Run only one time
	return FALSE;
}
DWORD GetDifference(DWORD dw1, DWORD dw2)
{
	if (dw1>dw2)
	{
		return dw1 - dw2;
	}
	else
	{
		return dw2 - dw1;
	}
}
//////////////////////////////////////////////////////////////////////
BOOL CBackupThread::RunCalculateEnd()
{
	CIPCInt64 s1=0,s2 = 0;
	CIPCInt64 one = 0;
	CIPCInt64 max = m_pBackup->GetMaximumInMB();
	max *= 1024*1024;
	DWORD dwTick = GetTickCount();
	CClient* pClient = theApp.m_Clients.GetClient(m_pBackup->GetClientID());
	CDWordArray dwIDs;
	CSystemTime start,t1,t2;
	CSequences sequences;


	// first check the IDs no Sequence allowed
	for (int i=0;i<m_pBackup->GetNrOfIDs();i++)
	{
		DWORD dwID = m_pBackup->GetIDNr(i);
		WORD wArchiveNr   = HIWORD(dwID);
		WORD wSequenceNr = LOWORD(dwID);
		if (wSequenceNr !=0)
		{
			pClient->GetCIPC()->DoIndicateError(CIPC_DB_REQUEST_BACKUP_BY_TIME,dwID,CIPC_ERROR_INVALID_VALUE,0);
			return FALSE;
		}
		TRACE(_T("RunCalculateEnd add id %08lx\n"),dwID);
		dwIDs.Add(dwID);
		CAutoCritSec acs(&theApp.m_Archives.m_cs);
		CCollection* pArchive = theApp.m_Archives.GetCollection(wArchiveNr);
		if (   pArchive
			&& !pArchive->IsBackup())
		{
			for (int j=0;j<pArchive->GetNrTapes();j++)
			{
				CTape* pSequence = pArchive->GetAtFast(j);
				if (!pSequence->IsBackup())
				{
					sequences.Add(pSequence);
				}
				if (m_bCancelled)
				{
					pClient = theApp.m_Clients.GetClient(m_pBackup->GetClientID());
					if (   pClient
						&& pClient->IsConnected())
					{
						pClient->GetCIPC()->DoConfirmCancelBackup(m_pBackup->GetUserData());
					}
					break;
				}
			}
		}
		acs.Unlock();

		if (m_bCancelled)
		{
			pClient = theApp.m_Clients.GetClient(m_pBackup->GetClientID());
			if (   pClient
				&& pClient->IsConnected())
			{
				pClient->GetCIPC()->DoConfirmCancelBackup(m_pBackup->GetUserData());
			}
			break;
		}
	}

	WK_TRACE(_T("CALCULATE BACKUP END sequences evaluated in %d ms\n"),GetTimeSpan(dwTick));
	sequences.Sort(m_bCancelled);
	WK_TRACE(_T("CALCULATE BACKUP END sequences sorted in %d ms\n"),GetTimeSpan(dwTick));

	start = m_pBackup->GetStartTime();

	for (i=0;i<sequences.GetSize();i++)
	{
		CTape* pSequence = sequences.GetAtFast(i);
		// cut only in front
		one = pSequence->GetSizeBytes(start,pSequence->GetLastTime());
		s1 = s2;
		s2 += one;
		if (m_bCancelled)
		{
			pClient = theApp.m_Clients.GetClient(m_pBackup->GetClientID());
			if (   pClient
				&& pClient->IsConnected())
			{
				pClient->GetCIPC()->DoConfirmCancelBackup(m_pBackup->GetUserData());
			}
			break;
		}
		pClient->GetCIPC()->DoConfirmBackupByTime((WORD)m_pBackup->GetUserData(),
												  MAKELONG(pSequence->GetNr(),pSequence->GetArchiveNr()),
												  pSequence->GetFirstTime(),
												  pSequence->GetLastTime(),
												  (WORD)BBT_CALCULATE_END,
												  one);
		if (s2 > max)
		{
			t2 = pSequence->GetLastTime();
			break;
		}
		t1 = pSequence->GetFirstTime();
		TRACE(_T("%d Sequences %04hx,%04hx %s Bytes %s\n"),i,
			pSequence->GetArchiveNr(),pSequence->GetNr(),s2.Format(TRUE),t1.GetDateTime());
	}

	if (s2 < max)
	{
		t2.GetLocalTime();
	}

	WK_TRACE(_T("CALCULATE BACKUP END in %d ms MinMB: %s  MaxMB: %s , %s->%s\n"),
		GetTimeSpan(dwTick),
		s1.Format(TRUE),
		s2.Format(TRUE),
		t1.GetDateTime(),
		t2.GetDateTime());

	pClient->GetCIPC()->DoConfirmBackupByTime((WORD)m_pBackup->GetUserData(),
											  0,
											  t1,
											  t2,
											  (WORD)m_pBackup->GetFlags(),
											  s2);

	return FALSE;
}
//////////////////////////////////////////////////////////////////////
void CBackupThread::Cancel(BOOL bByClient)
{
	m_bCancelled = TRUE;
	if (!bByClient)
	{
		DoIndicateError(5,0);
	}
}
//////////////////////////////////////////////////////////////////////
void CBackupThread::DoIndicateError(int iUnitCode, DWORD dwID)
{
	CClient* pClient;
	pClient = theApp.m_Clients.GetClient(m_pBackup->GetClientID());
	if (pClient)
	{
		CIPCDatabaseStorage* pCIPCDatabase = pClient->GetCIPC();
		pCIPCDatabase->DoIndicateError(
						 CIPC_DB_REQUEST_BACKUP, 
						 dwID, 
						 CIPC_ERROR_BACKUP, 
						 iUnitCode);
	}
}
//////////////////////////////////////////////////////////////////////
BOOL CBackupThread::IsCancelled()
{
	return m_bCancelled;
}
//////////////////////////////////////////////////////////////////////
BOOL CBackupThread::IsTimedOut()
{
	BOOL bRet = FALSE;
	CAutoCritSec acs(&m_csTimeOut);
	if (!m_bFinished)
	{
		DWORD dwDiff = GetTimeSpan(m_dwLastAction);
		bRet = (dwDiff>m_dwTimeOut);
	}
	return bRet;
}
//////////////////////////////////////////////////////////////////////
void CBackupThread::HandleTimeOut()
{
	if (m_dwTimeOutHandled == 0)
	{
		WK_TRACE(_T("backup time out %d\n"),m_pBackup->GetClientID());
		DoIndicateError(4,m_dwID);
		m_dwTimeOutHandled = GetTickCount();
	}
	else
	{
		if (GetTimeSpan(m_dwTimeOutHandled)>5*1000)
		{
			WK_TRACE(_T("exit by backup timeout\n"));
			theApp.EmergencyExit();
		}
	}
}
//////////////////////////////////////////////////////////////////////
CSequenceSize::CSequenceSize(CTape& s, CBackup& b)
{
	m_wArchiveNr  = s.GetArchiveNr();
	m_wSequenceNr = s.GetNr();

	if (s.GetFirstTime() < b.GetStartTime())
	{
		m_stStart = b.GetStartTime();
	}
	else
	{
		m_stStart = s.GetFirstTime();
	}

	if (s.GetLastTime() > b.GetEndTime())
	{
		m_stEnd = b.GetEndTime();
	}
	else
	{
		m_stEnd = s.GetLastTime();
	}

	m_i64SizeInBytes = s.GetSizeBytes(m_stStart,m_stEnd);
}
