...
CQueryRectTracker::CQueryRectTracker(CWndSmall* pWndSmall)
{
	m_pWndSmall = pWndSmall;
	m_rect.SetRectEmpty();
...
void CQueryRectTracker::DrawNewRect(CPoint point)
{
	...
	if(m_pWndSmall)
	{
		TrackRubberBand((CWnd*)m_pWndSmall, point, FALSE);
	}
...
//	TRACE(_T("tkr ---- OnChangedRect() \n"));
	CRect rc;
	ASSERT_KINDOF(CWndImage, m_pWndSmall);
	((CWndImage*)m_pWndSmall)->GetVideoClientRect(rc);

	//hat sich VideoClient window Größe geändert, neue Größe holen und sichern
	if(m_rcLastVideoClientRect != rc)
	{
		m_rcLastVideoClientRect = rc;
	}
		...
		|| (m_rect.bottom - m_rcLastVideoClientRect.bottom) > 0)
	{
		 ...
		 if(((CWndPlay*)m_pWndSmall)->GetZoom().IsRectEmpty())
		 {
			m_rect = rectOld;
		 }
		 else
		 {
			...
			CRect rcVideoClient;
			((CWndImage*)m_pWndSmall)->GetVideoClientRect(rcVideoClient);
			CRect rcZoom = ((CWndPlay*)m_pWndSmall)->GetZoom();
...
void CQueryRectTracker::RecalcNewRectPositionFromPromille(CRect rcZoom)
{
	...
	((CWnd*)m_pWndSmall)->GetClientRect(rcClient);
	((CWndImage*)m_pWndSmall)->GetVideoClientRect(rcVideoClient);
...
BOOL CQueryRectTracker::SetMinimumRect(BOOL bTrack)
{
	...
	if(!((CWndPlay*)m_pWndSmall)->GetZoom().IsRectEmpty())
	{
		CSize pic = ((CWndPlay*)m_pWndSmall)->GetPictureSize();
		CRect rcPicture(0,0,pic.cx, pic.cy);
		iZoom = rcPicture.Width()/((CWndPlay*)m_pWndSmall)->GetZoom().Width();
	}
...
{
	CRect rcClient, rcVideoClient;
	m_pWndSmall->GetClientRect(rcClient);
	ASSERT_KINDOF(CWndPlay, m_pWndSmall);
	((CWndPlay*)m_pWndSmall)->GetVideoClientRect(rcVideoClient);
	SaveLastClientRect(rcClient);
	SaveLastVideoClientRect(rcVideoClient);
	
	TRACE(_T("TKR ------ ResizeRectangle() Recalc\n"));
	RecalcNewRectPositionFromPromille(((CWndPlay*)m_pWndSmall)->GetZoom());
	Draw(((CWndPlay*)m_pWndSmall)->GetDC());
}
