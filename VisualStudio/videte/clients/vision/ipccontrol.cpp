	...
	if (id == SECID_NO_ID)
	{
		if (m_pServer->IsControl())
		{
			if (0==sKey.CompareNoCase(_T("cam")))
			{
				...
				{
					CObjectView* pView = m_pServer->GetDocument()->GetObjectView();
						...
						if (camID.IsOutputID())
						{
							...
							if (dwServerData)
							{
								pView->PostMessage(WM_USER,
												   MAKELONG(WPARAM_CONTROL_SELECT_CAM,m_pServer->GetID()),
												   camID.GetID());
							}
							else
							{
								pView->PostMessage(WM_USER,
												   MAKELONG(WPARAM_CONTROL_UNSELECT_CAM,m_pServer->GetID()),
												   camID.GetID());
							}
						}
						else
			...
			else if (0==sKey.CompareNoCase(_T("fullscreen")))
			{
				...
				if (WK_IS_WINDOW(pMF))
				{
					if (dwServerData)
					{
						...
						if (!pMF->IsFullScreen())
						{
							pMF->PostMessage(WM_COMMAND,ID_VIEW_FULLSCREEN);
						}
					}
					else
					{
