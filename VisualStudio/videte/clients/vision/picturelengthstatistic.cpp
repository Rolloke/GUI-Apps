...
/////////////////////////////////////////////////////////////////////////////
CPictureLengthStatistic::CPictureLengthStatistic(CIPCOutputVision* pOutput)
{
	m_pOutput = pOutput;
	m_bDelayed = FALSE;
...
void CPictureLengthStatistic::AddPictureLength(int iPictureLength)
{
	...
	if (m_AllTime!=0)
	{
		...
		if ((curTC-m_LastUpdated)>PANIC_SYNC_DIFF)
		{
			Reset();
		}
		...
		if (curTC-m_MedTime>m_dwMedBitTick)
		{
			m_MedBitrate = (m_MedDataSum*8000) / (curTC - (m_MedTime));
			if (theApp.m_bTraceBitrate)
			{
		...
		if (curTC-m_CurTime>m_dwCurBitTick)
		{
			m_pOutput->DoRequestSync(curTC, 0, 0);
			TRACE(_T("PILS DoRequestBitrate\n"));
			m_pOutput->DoRequestBitrate();
			m_CurBitrate = (m_CurDataSum*8000) / (curTC - m_CurTime);
			if (theApp.m_bTraceBitrate)
			{
				WK_STAT_LOG(szBitrate,(int)m_CurBitrate,_T("5sBitrate"));
				WK_STAT_LOG(szBitrate,(int)(m_dwPics*1000)/(curTC - m_CurTime),_T("FPS"));
			}
			m_CurDataSum = 0;
			m_dwPics = 0;
			m_CurTime = curTC;
...
void CPictureLengthStatistic::CalculateAverageBitrate()
{
	...
	if (m_AllDataSum>0)
	{
		m_AllBitrate = (m_AllDataSum*8000) / (curTC - m_AllTime);
		sum += m_AllBitrate;
		n++;
	}
