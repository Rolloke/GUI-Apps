...
class CAudioPropertiesDlg : public CDialog
{
// Construction
public:
	CAudioPropertiesDlg(CIPCAudioVision*pInput,CIPCAudioVision*pOutput, CWnd* pParent = NULL);   // standard constructor

// Dialog Data
	//{{AFX_DATA(CAudioPropertiesDlg)
	enum { IDD = IDD_AUDIO_PROPERTIES };
	CSliderCtrl	m_cThreshold;
	CSliderCtrl	m_cOutputVolume;
	CSliderCtrl	m_cInputSensitivity;
...
// Overrides
	...
	//{{AFX_VIRTUAL(CAudioPropertiesDlg)
	protected:
...
private:
	LRESULT OnNotifyWindow(CIPCAudioVision*, WPARAM, LPARAM);
	void InitCtrls();
	CIPCAudioVision *m_pControlInput;
	CIPCAudioVision	*m_pControlOutput;
	CSecID			m_SecIDinput;
