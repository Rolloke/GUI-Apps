...
CQueryDialog::CQueryDialog(CObjectView* pParent /*=NULL*/)
	: CWK_Dialog(CQueryDialog::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(CQueryDialog, CWK_Dialog)
...
void CQueryDialog::PostNcDestroy() 
{
	m_pObjectView->m_pQueryDialog = NULL;
	delete this;
}
/////////////////////////////////////////////////////////////////////////////
BOOL CQueryDialog::OnInitDialog() 
{
	...
	int iSize = m_pObjectView->GetDocument()->GetRechercheView()->GetSmallWindows()->GetSize();
	...
	if(iSize)
	{
		...
		{
			pSW = m_pObjectView->GetDocument()->GetRechercheView()->GetSmallWindows()->GetAt(i);
			if(pSW)
			{
...
void CQueryDialog::InitFields()
{
	...
	for (i=0;i<pDoc->GetServers().GetSize();i++)
	{
		...
		if (pServer && pServer->IsConnected())
		{
			...
			{
				CIPCField* pField = theApp.GetQueryParameter().GetFields().GetAt(j);
	...
	CString sName;
	CQueryFieldDialog* pQueryFieldDialog;

	for (i=0;i<c;i++)
	{
		CIPCField* pFieldName = m_FieldNames.GetAt(i);
		...
		if (!sName.IsEmpty())
		{
			...
			{
				pQueryFieldDialog = new CQueryFieldDateDialog(sName,pFieldValue,this);
			}
			else if (pFieldName->IsTime())
			{
				pQueryFieldDialog = new CQueryFieldTimeDialog(sName,pFieldValue,this);
			}
			else if (pFieldName->IsNumeric())
			{
				pQueryFieldDialog = new CQueryFieldNumDialog(sName,pFieldValue,this);
			}
			else
			{
				pQueryFieldDialog = new CQueryFieldCharDialog(sName,pFieldValue,this);
			}
...
void CQueryDialog::Resize()
{
	...
	if (c>0)
	{
		pQueryFieldDialog = m_QueryFieldDialogs.GetAt(0);
		...
		for (i=0;i<c;i++)
		{
			pQueryFieldDialog = m_QueryFieldDialogs.GetAt(i);
			pQueryFieldDialog->GetClientRect(rect);
			height = rect.Height();
...
/////////////////////////////////////////////////////////////////////////////
void CQueryDialog::Disconnect(DWORD dwServerID)
{
...
/////////////////////////////////////////////////////////////////////////////
void CQueryDialog::InitServerArchives()
{
...
void CQueryDialog::CheckDateTime() 
{
...
void CQueryDialog::OnOK() 
{
	CheckDateTime();
	Parameters2QueryParameter();
	m_pObjectView->ShowSearchResultDialog();
	StartSearch();
	DestroyWindow();
}
/////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////
void CQueryDialog::OnCheckAll() 
{
	UpdateData();
	ShowHide();
}
/////////////////////////////////////////////////////////////////////////////
void CQueryDialog::StartSearch()
{
	HTREEITEM hServer = m_ServerArchives.GetRootItem();
	while (hServer)
	{
		WORD wServerID = (WORD)m_ServerArchives.GetItemData(hServer);
		CServer* pServer = m_pObjectView->GetDocument()->GetServer(wServerID);
		if (pServer && pServer->IsConnected())
		{
...
/////////////////////////////////////////////////////////////////////////////
void CQueryDialog::Parameters2QueryParameter()
{
	UpdateData();
	...
	{
		CIPCField* pField = m_FieldValues.GetAt(i);
...
/////////////////////////////////////////////////////////////////////////////
void CQueryDialog::OnClickServerArchives(NMHDR* pNMHDR, LRESULT* pResult) 
{
	CPoint pt;
	GetCursorPos(&pt);
...
//	TRACE(_T("TKR ----------------------- RectIsNull: %i  All: %i  ServerChecked: %i\n"), !bRectIsNull,m_bAll,m_iServersChecked);
	...
	m_ctrlCopyPictures.ShowWindow(theApp.m_bReadOnlyModus ? SW_HIDE : SW_SHOW);
	m_ctrlSearchArea.ShowWindow(SW_SHOW);

}

/////////////////////////////////////////////////////////////////////////////
void CQueryDialog::ShowHideRectangle(BOOL bShow)
{
...
/////////////////////////////////////////////////////////////////////////////
void CQueryDialog::ShowHideServerArchives(BOOL bShow)
{
	if(bShow) //hier rein, wenn Rechteck auf Fenster aufgezogen ist
	{
		...
		if(iSize == 1)
		{
			pSW = m_pObjectView->GetDocument()->GetRechercheView()->GetSmallWindows()->GetAt(0);
		}
		else if(iSize > 1)
		{
			CDisplayWindow* pDW = NULL;
		...
		if(pSW)
		{
			...
			if(pDW)
			{
				...
				CString sArchivName = pSW->GetServer()->GetArchivName(wArchivNr);
...
/////////////////////////////////////////////////////////////////////////////
void CQueryDialog::OnCheckExactTime() 
{
	UpdateData();
...
/////////////////////////////////////////////////////////////////////////////
void CQueryDialog::OnCheckTimeFrom() 
{
	UpdateData();
	ShowHide();
}
/////////////////////////////////////////////////////////////////////////////
void CQueryDialog::SetPageSize(int iSize)
{
...
/////////////////////////////////////////////////////////////////////////////
void CQueryDialog::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
