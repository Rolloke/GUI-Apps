...
$theDestinationDir="v:\\bin\\dv";	# has to contain vinfo.pvi for the buildNumber info
$theDestinationDirEnu="v:\\bin\\dv\\enu";
...
# obsolete			  "Lib/JpegLib",
			  ...
			  "Lib/DeviceDetect",
# obsolete			  "Lib/h263dec",
# obsolete			  "Lib/h263enc",
			  "Lib/NetComm",
			  "Lib/HardwareInfo",
			  "DV/Lib/Hooks",
...
{
	print "DVMake started at $mday.$month.$year, $hour:$min:$sec ; Versionlabel '$VersionLabel' ; compiling for '$versionInfo$currentBuild'\n";
	if ($bGetLibs)
	{
		#
		# get imagn32.lib
...
#			print "else MyUnlink $project\n";
	...
	&SetTimeVariables();
	$MakeDoneText = "DVMake DONE at $mday.$month.$year, $hour:$min:$sec, Version $versionInfo$currentBuild";
	print "\n$MakeDoneText\n";
	...
	&SetTimeVariables();
	$MakeEndText = "DVMake END at $mday.$month.$year, $hour:$min:$sec, Version $versionInfo$currentBuild";
	print "\n$MakeEndText\n";
}
...
#	bWorkAsSuperCopy == 1
	...
	print "DVCopy started at $mday.$month.$year, $hour:$min:$sec ; Versionlabel '$VersionLabel' ; copying for '$versionInfo$currentBuild'\n";
	# switch to copy mode
	$bInFinalCopy = 1;	
	...
	$MakeDoneText = "DVCopy DONE at $mday.$month.$year, $hour:$min:$sec, Version $versionInfo$currentBuild";
	print "\n$MakeDoneText\n";
	...
	&SetTimeVariables();
	$MakeEndText = "DVCopy END at $mday.$month.$year, $hour:$min:$sec, Version $versionInfo$currentBuild";
	print "\n$MakeEndText\n";
}
...
#	print "Ready\n";
	...
	print "Creating g:\\BuildDV\\Build$currentBuild.zip\n";
    print "executing $ZIP_CMD -add -recurse -directories -silent g:\\BuildDV\\Build$currentBuild.zip $theDestinationDir\\*.*\n";
	`$ZIP_CMD -add -recurse -directories -silent g:\\BuildDV\\Build$currentBuild.zip $theDestinationDir\\*.*`;
}
#######################################################################
sub DoOneConfiguration
{
	...
	# do not remove to avoid timestamp differences &MyUnlink("$theProjectDir/lib/$dest.dll",0);
	# do not remove to avoid timestamp differences &MyUnlink("$theProjectDir/$project/$destDir/$dest.exe",0);
...
{ 
	...
	if ($project eq "SecurityServer") {
		...
		$mak =~  s/^Units\/SaVic2\///;	# drop leading Units/SaVic/SaVicUnit
	...
	elsif (    $project eq "Tools/LogView"
...
sub DoCopy
{
		...
		) 
	{
	...
	} elsif (/WKClasses/ || /jpeg/ || /C4Lib/ || /pil/ || /baseclasses/) {
		$bDoCopy=0;
	} elsif (/h263dec/) {
		...
		$source = "$project\\Release\\JaCob.dll";
		...
		$source = "$project\\Release\\SaVic.dll";
		...
		{
			$source = "$theProjectDir\\Lib\\$dest\\$config\\$dest.dll";
		}
		$finalDest = "$destinationDir\\Dlls\\$dest.dll";
	} else {
		#
...
sub CopyBuildNotes
{
	...
	$dest   = "\\\\mail01\\buildnotes\\dts\\BuildNotes$currentBuild.htm";
