...
&SetTimeVariables();
print "DVWise started at $mday.$month.$year, $hour:$min:$sec\n";
...
if (!(-f "g:\\BuildDV\\Build$BUILD\.zip"))
{
	print "g:\\BuildDV\\Build$BUILD\.zip not found\n";
	die;
}
...
chdir($DEST_DIR);
# creates bin
print "executing $ZIP_CMD -extract -recurse -directories g:\\BuildDV\\Build$BUILD\.zip\n";
print `$ZIP_CMD -extract -recurse -directories g:\\BuildDV\\Build$BUILD\.zip`;
chdir("D:");
chdir("/");
...
`$WISE_CMD /c \"$WISE_DIR\\dts\\dts_transmitter.wse\" `;
`$WISE_CMD /c \"$WISE_DIR\\dts\\dts_receiver.wse\" `;
`$WISE_CMD /c \"$WISE_DIR\\dts\\dts_ip_receiver.wse\" `;
`$WISE_CMD /c \"$WISE_DIR\\dts\\dts_tools.wse\" `;
...
chdir("$INSTALL_DIR");
`$ZIP_CMD -add -recurse -directories -silent g:\\BuildDV\\Build$BUILD\.zip $INSTALL_DIR\\Alarmcom\\*.*`;
`$ZIP_CMD -add -recurse -directories -silent g:\\BuildDV\\Build$BUILD\.zip $INSTALL_DIR\\Dresearch\\*.*`;
`$ZIP_CMD -add -recurse -directories -silent g:\\BuildDV\\Build$BUILD\.zip $INSTALL_DIR\\dts\\*.*`;
`$ZIP_CMD -add -recurse -directories -silent g:\\BuildDV\\Build$BUILD\.zip $INSTALL_DIR\\Santec\\*.*`;
`$ZIP_CMD -add -recurse -directories -silent g:\\BuildDV\\Build$BUILD\.zip $INSTALL_DIR\\Pone\\*.*`;
...
# copying version.txt
`copy /Y $DEST_DIR\\version.txt $INSTALL_DIR\\Dts\\version.txt`;
`copy /Y $DEST_DIR\\version.txt $INSTALL_DIR\\alarmcom\\version.txt`;
`copy /Y $DEST_DIR\\version.txt $INSTALL_DIR\\pone\\version.txt`;
`copy /Y $DEST_DIR\\version.txt $INSTALL_DIR\\dresearch\\version.txt`;
`copy /Y $DEST_DIR\\version.txt $INSTALL_DIR\\santec\\version.txt`;

$WiseDoneText = "DVWise done for build $BUILD";
&SetTimeVariables();
print "$WiseDoneText at $mday.$month.$year, $hour:$min:$sec\n";

$NewVersionText = "Neue DV Version!";
...
&SetTimeVariables();
print "DVWise end $NewVersionText at $mday.$month.$year, $hour:$min:$sec\n";
###########################################################
# end of main
