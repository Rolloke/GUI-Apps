...
$WorkAs = "SuperMake";
$bInFinalCopy = 0;
$errorCountCopy=0;	# modified in MyDoCopy
$errorCount=0;
$warningCount=0;

$theDestinationDir="v:/bin/Akubis_5_2";			# has to contain vinfo.pvi for the buildNumber info
...
# >>> modify this line for new Version <<<
$VersionMajor=5;
$VersionMinor=2;
$VersionSubIndex=0;
...
@LibProjects=(
				"Project/Lib/WKClasses",	# needed for cipc!
				"Project/Cipc",
				"Project/Lib/oemgui",
				);
...
{
	print "AkubisMake_5_2 started at $mday.$month.$year, $hour:$min, label was '$VersionLabel', compiling for $versionInfo, build $currentBuild\n";
	if ($bGetLibs)
	{
...
#			die;
	...
	&SetTimeVariables();
	print "AkubisMake_5_2 DONE at $mday.$month.$year, $hour:$min, label was '$VersionLabel', compiling for $versionInfo, build $currentBuild\n";
	...
	`Net Send Georg "AkubisMake_5_2 DONE at $mday.$month.$year,  $hour:$min,  version $versionInfo build $currentBuild ; $errorCount error[s], $warningCount warning[s]"`;
}
elsif ($WorkAs eq "SuperCopy")
{
	print "AkubisCopy_5_2 started at $mday.$month.$year, $hour:$min, version $versionInfo$currentBuild\n";
	# switch to copy mode
	$bInFinalCopy = 1;	
...
#	&CreateNewDir("$theDestinationDirEnu/Maps");
	...
	{
		print ("\n");
		print ("Es gab $errorCountCopy Fehler beim Kopieren\n");
	...
	&SetTimeVariables();
	print "AkubisCopy_5_2 DONE at $mday.$month.$year, $hour:$min, version was '$versionInfo$currentBuild'\n";
	print "Net Send Georg \n";
	`Net Send Georg "AkubisCopy_5_2 DONE at $mday.$month.$year,  $hour:$min,  version $versionInfo build $currentBuild ; $errorCountCopy error[s], $warningCount warning[s]"`;
}
...
sub UpdateBuild
{
	...
	print `$SAFE_CMD  Get \"\\$\\Telekom\\Installation\\Evaluation_5_2.wse" -I- \"-VL$VersionLabel\"`;
	`$WISE_CMD /c \"$theProjectDrive\\Telekom\\Installation\\Evaluation_5_2.wse\" `;
	print `$SAFE_CMD  Get \"\\$\\Telekom\\Installation\\MasterControl_5_2.wse" -I- \"-VL$VersionLabel\"`;
	`$WISE_CMD /c \"$theProjectDrive\\Telekom\\Installation\\MasterControl_5_2.wse\" `;
...
#			|| $project eq "Project/Lib/Network"
			|| $project eq "Telekom/MasterControl_5_2"
