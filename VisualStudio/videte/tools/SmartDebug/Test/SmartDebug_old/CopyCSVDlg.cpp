...
int CCopyCSVDlg::DoModal(HWND hWndParent)
{
   if (_AtlModule.GetResourceInstance() != NULL)
   {
      ...
      return (int)DialogBoxParam(_AtlModule.GetResourceInstance(),
         MAKEINTRESOURCE(IDD_WATCH_ARRAY), hWndParent,
...
bool CCopyCSVDlg::OnInitDialog(HWND hWnd)
{
   ...
   gm_oldDrawGraphProc =(WNDPROC)(LONG_PTR)SetWindowLongPtr(GetDlgItem(m_hWnd, IDC_WND_GRAPH), GWL_WNDPROC, (LONG)(LONG_PTR)DrawGraphProc);
   ::SetWindowLongPtr(GetDlgItem(m_hWnd, IDC_WND_GRAPH), GWL_USERDATA, (LONG)(LONG_PTR)this);
...
int CCopyCSVDlg::OnCommand(WORD nID , WORD nNotifyCode, HWND hwndControl)
{
   if (nNotifyCode == BN_CLICKED)
   {
      ...
      else if (nID == IDC_BTN_COPY_VALUES)
      {
            int i, nSize = 0;
            for (i=0; i<m_nPoints; i++)
            {
...
BOOL CALLBACK CCopyCSVDlg::MsgFilterEnum(HWND hwnd, LPARAM lParam)
{
   ...
   if (wcscmp(szName, L"#32770") == 0)
   {
      ...
      {
         CCopyCSVDlg* pDlg = (CCopyCSVDlg*)(LONG_PTR)::GetWindowLong(hwnd, DWL_USER);
         if (pDlg)
         {
...
HWND CCopyCSVDlg::Create(HWND hWndParent)
{
   ...
   else
   {
      ...
      {
         m_bModal = false;
         return CreateDialogParam(_AtlModule.GetResourceInstance(), MAKEINTRESOURCE(IDD_WATCH_ARRAY), m_hWndParent,
                               (DLGPROC) DialogProc, (LPARAM) this);
      }
      return NULL;
   }
...
{
   CCopyCSVDlg *pThis = (CCopyCSVDlg*)(LONG_PTR)GetWindowLongPtr(hWnd, GWL_USERDATA);
   switch (Msg)
   {
      case WM_PAINT:
      {
