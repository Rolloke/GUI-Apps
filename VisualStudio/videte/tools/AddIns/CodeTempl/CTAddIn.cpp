...
HRESULT CCTAddIn::OnConnection(IApplication* pApp, VARIANT_BOOL bFirstTime, long dwAddInID, VARIANT_BOOL* bOnConnection)
{
	...
	pApp->PrintToOutputWindow(CComBSTR(_T("Foo Msg")));
...
HRESULT CCTAddIn::CTInsCodeTmpl()
{
    try
    {
        ...
        if (nId > 0)
        {
			...
			// HEDU
			spTextSel->MoveTo(lStartLine, lEndColumn, CComVariant(dsMove)); // column + 1 ?
            // ORIG spTextSel->MoveTo(lEndLine, lEndColumn, CComVariant(dsMove));
        }
    }
...
void CCTAddIn::ReadTmplFile()
{
    ...
    // HEDU string strMSDevDir = getenv("MSDevDir");
	string strMSDevDir = "V:\\Bin\\Tools";	// HEDU
    string strTmplFile = strMSDevDir + "\\CodeTmpl.txt";

	AtlTrace("Found MSdDvdir='%s' \n",getenv("MSDevDir"));
    //
    ...
    while (ifs)
    {
        ...
        else 
        {
            ...
            if (strLine.length() >= 2 && strLine.substr(0, 2) == "#}" ) 
            {
				...
				// HEDU drop the trailing \r\n, only one!
				if (td.strText.length()>1) {
					td.strText.replace(td.strText.length()-2,2,"");
				}
