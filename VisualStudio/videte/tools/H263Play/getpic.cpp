// getpic.cpp, picture decoding for tmndecode (H.263 decoder)
...
void CH263Decoder::getpicture(int *framenum)
{
	int i;
	unsigned char *tmp;
	...
	{
		TRACE("make_edge_image\n");
		make_edge_image(m_oldrefframe[0],m_edgeframe[0],coded_picture_width,
            coded_picture_height,32);
		make_edge_image(m_oldrefframe[1],m_edgeframe[1],chrom_width, chrom_height,16);
		make_edge_image(m_oldrefframe[2],m_edgeframe[2],chrom_width, chrom_height,16);
	}
	getMBs(*framenum);
...
/*	
	...
	if (pb_frame) 
	{
		TRACE("pb_frame displayImage");
		displayImage(m_bframe[0],m_bframe[2],m_bframe[1]);
		*framenum += pb_frame;
	}
*/
	displayImage(m_newframe[0],m_newframe[2],m_newframe[1]);
}


/* decode all macroblocks of the current picture */

void CH263Decoder::getMBs(int framenum)
{
	int comp;
	int MBA, MBAmax;
	int bx, by;
...
it is necessary to include this in the freely available
		...
		if (fault) {
			TRACE("Warning: A Fault Condition Has Occurred - Resyncing \n");
			startcode();  /* sync on new startcode */
			fault = 0;
		}
		
		if (!(m_Bits.showbits(22)>>6)) { /* startcode */
			
			startcode();  
			/* in case of byte aligned start code, ie. PSTUF, GSTUF or ESTUF
			is used */
			
			if (m_Bits.showbits(22) == (32|SE_CODE)) { /* end of sequence */
				if (!(syntax_arith_coding && MBA < MBAmax)) {
					return;
				}
			}
			else if ((m_Bits.showbits(22) == PSC<<5) ) { /* new picture */
				if (!(syntax_arith_coding && MBA < MBAmax)) {
					return;
				}
			}
				...
				if (!(syntax_arith_coding && MBA%mb_width)) {
					...
					if (syntax_arith_coding) {   /* SAC hack to finish GOBs which   */
						gob = (m_Bits.showbits(22) & 31); /* end with MBs coded with no bits */
						if (gob * mb_width != MBA) 
							goto finish_gob;
					}
...
read_cod:
		if (syntax_arith_coding) { 
			if (pict_type == PCT_INTER) {
				...
				if (trace) {
					TRACE("Arithmetic Decoding Debug \n");
					TRACE("COD Index: %d COD: %d \n", COD_index, COD);
				}
			}
			else
		...
		if (!COD) {  /* COD == 0 --> not skipped */    
			...
			if (syntax_arith_coding)  {
				...
				if (trace) 
					TRACE("MCBPC Index: %d MCBPC: %d \n",MCBPC_index, MCBPC);
			}
			
			else {
				if (pict_type == PCT_INTER)
					m_Bits.flushbits(1); /* flush COD bit */
			...
			if (Mode == MODE_INTER4V && !adv_pred_mode) 
				if (!m_bQuiet)
				{
					TRACE("8x8 vectors not allowed in normal prediction mode\n");
				}
				...
				if (Mode == MODE_INTER_Q || Mode == MODE_INTRA_Q) {
					...
					if (syntax_arith_coding) {
						...
						quant +=DQUANT;
						if (trace)
							TRACE("DQUANT Index: %d DQUANT %d \n",DQUANT_index, DQUANT);
					}
					else {
						DQUANT = m_Bits.getbits(2);
						quant += DQ_tab[DQUANT];
						if (trace) {
							TRACE("DQUANT (");
					...
					if (quant > 31 || quant < 1) {
						if (!m_bQuiet)
						{
							TRACE("Quantizer out of range: clipping\n");
						}
					...
					for (k = startmv; k <= stopmv; k++) {
						...
						if (!mv_outside_frame) {
							...
							if ((xpos<<4) + (mvx/2) + offset < 0 ||
								...
								if (!m_bQuiet)
								{
									TRACE("mvx out of range: searching for sync\n");
								}
								fault = 1;
							}
							offset = k ? (((k-1)&2)<<2) : 0;
							if ((ypos<<4) + (mvy/2) + offset < 0 ||
								...
								if (!m_bQuiet)
								{
									TRACE("mvy out of range: searching for sync\n");
								}
								fault = 1;
							}
						}
			...
			for (comp=0; comp<blk_cnt; comp++) {
				...
				if (Mode == MODE_INTRA || Mode == MODE_INTRA_Q) { /* Intra */
					...
					if(syntax_arith_coding) {
						...
						if (trace)
							TRACE("INTRADC Index: %d INTRADC: %d \n", INTRADC_index, bp[0]);
					}
					else {
						bp[0] = m_Bits.getbits(8);
						if (trace) {
							TRACE("DC[%d]: (",comp);
					...
					if (bp[0] == 128)
						if (!m_bQuiet)
						{
							TRACE("Illegal DC-coeff: 1000000\n");
						}
...
void CH263Decoder::clearblock(int comp)
{
	int *bp;
	int i;
...
/* move/add 8x8-Block from block[comp] to m_refframe or m_bframe */

void CH263Decoder::addblock(int comp,int bx,int by,int addflag)
{
	int cc,i, iincr, P = 1;
	unsigned char *rfp;
	short *bp;
	...
	if (cc==0) {
		...
		/* frame DCT coding */
		if (P)
			rfp = m_newframe[cc] + chrom_width*by + bx;
		else
			rfp = m_bframe[cc] + chrom_width*by + bx;
		iincr = chrom_width;
	}
...
/* bidirectionally reconstruct 8x8-Block from block[comp] to m_bframe */

void CH263Decoder::reconblock_b(int comp,int bx,int by,int mode,int bdx,int bdy)
{
	int cc,i,j,k, ii;
	...
	if (cc==0) {
		...
		bfr = m_bframe[cc]      + chrom_width*(by+((comp&2)<<2)) + bx + (comp&8);
		ffr = m_newframe[cc]    + chrom_width*(by+((comp&2)<<2)) + bx + (comp&8);
		ii = chrom_width;
	}
...
int CH263Decoder::motion_decode(int vec,int pmv)
{
	if (vec > 31) vec -= 64;
	vec += pmv;
	if (!long_vectors) {
...
int CH263Decoder::find_pmv(int x, int y, int block,int comp)
{
	int p1,p2,p3;
	int xin1,xin2,xin3;
	int yin1,yin2,yin3;
