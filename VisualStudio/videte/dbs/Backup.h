...
public:
	...
	inline BOOL RemoveAllFurtherIDsFromList(int iIndex);

	// operations
public:
	BOOL Initialize();
	BOOL WasLastSequenceOfArchive(WORD wArchive, WORD wSequence);
	BOOL CopyFileTrace(WORD wArchiv, WORD wSequence, 
						const CString& source, 
						const CString& dest, 
						FILESIZE dwMaxRead=-1);
	void IncreaseCopiedBytes(WORD wArchiv, WORD wSequence, FILESIZE dwBytes,WORD wFlags);
	BOOL CleanUp();
	BOOL CleanUpPartly();
	BOOL CleanUpPartly_Files();
	void WriteSequenceMap();
	void WriteCameraMaps();
	static void WriteBackupDbi(const CString& sPathName,
								const CString& sName,
...
protected:
	...
	BOOL MyCopyFile(WORD wArchive, WORD wSequence, const CString& sSource, const CString& sDest, FILESIZE dwMaxRead=-1);
...
//////////////////////////////////////////////////////////////////////
//delete all IDs from the iIndex until the end.
inline BOOL CBackup::RemoveAllFurtherIDsFromList(int iIndex)
{
	BOOL bRet = FALSE;
	if (iIndex>=0 && iIndex<m_IDs.GetSize())
	{
		m_IDs.SetSize(iIndex);
		bRet = TRUE;
	}
	return bRet;
}
