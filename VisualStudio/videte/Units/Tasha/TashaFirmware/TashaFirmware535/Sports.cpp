...
// FILE:		$Workfile: Sports.cpp $
// ARCHIVE:		$Archive: /Project/Units/Tasha/TashaFirmware/TashaFirmware535/Sports.cpp $
// CHECKIN:		$Date: 19.08.04 13:04 $
// VERSION:		$Revision: 67 $
// LAST CHANGE:	$Modtime: 19.08.04 13:04 $
...
void EnableSPORTs()
{
	...
	*pSPORT0_RFSDIV = SPORT0_CHANNELS*SPORT0_TIME_SLICE_SIZE*8; // nach 4*32 SClock Framewechsel 
	*pSPORT1_RFSDIV = SPORT1_CHANNELS*SPORT1_TIME_SLICE_SIZE*8; // nach 4*32 SClock Framewechsel 
	...
	*pSPORT0_MCMC1 = 0x0005;	// Ein clk-delay zum Ausgleich der laufzeiten + 1 clk-delay,
	*pSPORT1_MCMC1 = 0x0005;	// weil die BF533 einen clk-Framedelay benötigen!
...
void DisableSPORTsDMA()
{
	...
	*pSPORT1_CONFIG_DMA_RX  = *pSPORT1_CONFIG_DMA_RX | FLSH;	// Set FLSH-Bit
	WaitMicro(1); // Wait 4 SPORT clock cycles
...
{
	// Lasse die 2. LED regelmäßig blinken
	...
	if (TSTBIT(*pSPORT0_IRQSTAT_RX,0))
	{
		...
		if (g_bCaptureOn && !g_bBF533Booting && !g_bEEPromDataTransfer)
		{
...
///////////////////////////////////////////////////////////////////////////////////
// Am SPORT0 hängen die BF533 (1,3,5,7)
	...
	{		
						 			 ...
						 			 SPORT0_CHANNELS, FALSE))						 
		{
			...
			{	
				...
				WaitUntilMemCopyDmaCompleted(10000);
				...
				if (dwSDRAMOffset >= MAX_FRAME_BUFFERS*SDRAM_DMA_BUFFER0_SIZE)
					dwSDRAMOffset = 0;
				dwIndex = 0;
			}		
...
///////////////////////////////////////////////////////////////////////////////////
// Am SPORT1 hängen die BF533 (2,4,6,8)
						     ...
						     SPORT1_DMA_BUFFER_SIZE))
	{
							 		...
							 		SPORT1_CHANNELS, FALSE))
		{
			...
			{	
				...
				// Informiere die TashaUnit, darüber, daß sie die Daten der Kanäle 4...7 abholen kann
				DoIndicationNewData(dwSDRAMOffset, SPORT1);
				
				// Wechsel zwischen den beiden SDRAM-Buffern. Während die Bilddaten von der TashaUnit
				// abgeholt werden, schreibt der DMA-Prozess die Daten in einen zweiten Buffer.
				dwSDRAMOffset += SDRAM_DMA_BUFFER1_SIZE;
				if (dwSDRAMOffset >= MAX_FRAME_BUFFERS*(SDRAM_DMA_BUFFER0_SIZE+SDRAM_DMA_BUFFER1_SIZE))
					dwSDRAMOffset = MAX_FRAME_BUFFERS*SDRAM_DMA_BUFFER0_SIZE;
				dwIndex = 0;
			}
			bResult = TRUE;
		}	
