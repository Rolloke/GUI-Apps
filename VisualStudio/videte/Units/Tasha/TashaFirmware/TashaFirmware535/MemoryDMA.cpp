...
// FILE:		$Workfile: MemoryDMA.cpp $
// ARCHIVE:		$Archive: /Project/Units/Tasha/TashaFirmware/TashaFirmware535/MemoryDMA.cpp $
// CHECKIN:		$Date: 5.03.04 13:12 $
// VERSION:		$Revision: 17 $
// LAST CHANGE:	$Modtime: 5.03.04 13:00 $
...
BOOL MemCopyDma(BYTE* pDst, BYTE* pSrc, DWORD dwLen, BOOL bWait/*=TRUE*/)
{
	...
	if (pSrc && pDst)
	{
		...
		if (WaitUntilMemCopyDmaCompleted(10000))
		{
			...
			if (CreateSingleChannelDescriptorList(pDst, pSrc, dwLen))
			{
				...
				if (bWait)
					bResult = WaitUntilMemCopyDmaCompleted(10000);
				else
					bResult = TRUE;
			}
		}
...
{
	int nCounter = 0;
	while(!IsMemCopyDmaReadyEx() && (nCounter++ < nTimeOut))
		WaitMicro(1);
	return !TSTBIT(*pMDD_DCFG, 14); // DCS-Bit im StatusRegister	
}
