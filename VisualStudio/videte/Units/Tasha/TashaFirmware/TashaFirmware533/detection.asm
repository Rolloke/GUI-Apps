...
.extern _byMask_B;
.extern _byThresholdMD;		// Schwelle bei der Motion Detection
.extern _byThresholdMask;	// Schwelle bei der adaptiven Maske
.extern _byIncrementMask;	// Wert um die die Maske erhöht wird 
...
_max_search:
	...
	row_start:
		...
		col_start:
			...
			r1.l = r1.l * r2.l(fu);	// Differenzwert mit Maskenwert bewerten
			p2.l = _byThresholdMD;  	// Alarmschwelle in r2 laden
			p2.h = _byThresholdMD;
			r2 = b[p2](z);
...
_adaptiv_mask:
	...
	p3.l = _byMask_A;		// Adressen der adaptiven Maske_A  in p3 laden
	p3.h = _byMask_A;
	p1.l = _byIncrementMask;
    p1.h = _byIncrementMask;
	p2.l = _byThresholdMask; 
	p2.h = _byThresholdMask;
	p5 = (ROW/2)/BLOCK(z);	// Vertikal 36 Zeilen im Differenzbild
	p4 = (COL/2)/BLOCK(z);	// Horizontal 45 Spalten im Differenzbild
	...
	row_start1:
		...
		col_start1:			
			...
			r2 = b[p2](z);			// Schaltschwelle in r2
			...
			CC = r6 <= r2;			// Wenn Maskenwert = Maximalwert; CC = 1
    		if CC jump adaptiv_auf;	// Wenn CC = 1 Sprung zum Ende
...
mask_add:				// Maske erhöhen bedeutet ein Verringern des Maskenwertes
	...
	r2.l = r2.l - r6.l(ns); // Maskenwert um MOD verringern
   	r6 = 0xFF00(z);		// Minimalwert laden
	CC = r6 <= r2;		// Wenn Maskenwert kleiner als Minimalwert; CC = 1
...
_docheck:					// DWORD docheck(DWORD pIn, DWORD nLenght); 
	SAVE_ASM_REGS;			// Summiert den pIn Buffer auf und liefert das Ergebnis in nChecksum
	p1 = r1;
	p0 = r0;
	r1 = 0;
	nop;
	nop;
	lsetup(check_start, check_end) lc0=p1; 
	check_start:
		r2 = b[p0++](z);
	check_end: 	r1 = r1 + r2(ns);
	r0 = r1;	
_docheck.end:
RESTORE_ASM_REGS; 
rts;						// Zurück zum aufrufenden Programm
//////////////////////////////////////////////////////////////////////////////////////
/*
_makeit:					// Macht aus YUV 4:2:2 YUV 4:2:0 planar
	SAVE_ASM_REGS;			// R0 = FrameBuff R1 = Y, R2 = Cb, [FP+20] = Cr 
	p0.l = 0x0004;
	p0.h = 0x0000;
	p1 = r0;
	p2 = r1;
	...
	i0 = 0;
	i1 = 0;
	lsetup(make_start2, make_end2) lc1=p5;
	make_start2:
		lsetup(make_start, make_end) lc0=p4; 
		make_start:
			r0 = [p0++];
		...
		lsetup(make_start1, make_end1) lc0=p4; 
		make_start1:
			r0 = [p0++];
			(r6,r5) = byteunpack r1:0;
			r6 >>= 16;
