...
// ARCHIVE:		$Archive: /Project/Units/SaVic/SavicDll/SavicDefs.h $
...
// Versionsnummer von SaVicUnit, SaVicDA.dll, und SaVic.sys
#define SAVIC_UNIT_VERSION				0x00040026
...
#define FIELD_BUFFER_SIZE			768*288*2	// Maximale Größe des Inlaybuffers
...
typedef struct
{
	WORD		wQueueState[MAX_FIELD_BUFFER];	// Status des Queueeintrages
...
typedef struct	    
{
	...
	DWORD			dwReqPics;				// Geforderte Bildzahl
	DWORD			dwReqMinLen;			// Geforderte Mindestgröße in Byte
	DWORD			dwReqMaxLen;			// Geforderte Maximalgröße in Byte
	BOOL			bReqDoubleField;		// TRUE -> Vollbild (2Fields)
	WORD			wReqSource;				// Quelle auf die umgeschaltet werden soll.
	WORD			wReqPortYC;				// Port der Videoquelle, auf die geschaltet werden soll. (Dient zum evtl. mappen)
	WORD			wReqBrightness;			// Gewünschte Helligkeit
	WORD			wReqContrast;			// Gewünschter Kontrast
	WORD			wReqSaturation;			// Gewünschte Farbsättigung
	WORD			wReqHue;				// Gewünschte Farbbalance
	WORD			wReqCompressionType;	// Gewünschte Bildkompression (z.B. JPeg, DIB,...)

	DWORD			dwProzessID;			// Eindeutige ProzeßID des laufenden Jobs
	DWORD			dwPics;					// Bildzähler
	...
	BOOL			bFlock;					// TRUE -> Eintritt in die ISR
	...
	MEMSTRUCT		KDpcSourceSwitch;		// Pointer auf Dpc (WinNT);
	MEMSTRUCT		KDpcDebug;				// Pointer auf Dpc (WinNT);
	MEMSTRUCT		SpinLock;				// Pointer auf ISR-Spinlock
	MEMSTRUCT		OddInlayFieldBuffer;	// Adresse des Inlays für MotionDetection
