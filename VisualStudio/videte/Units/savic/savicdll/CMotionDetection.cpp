...
// ARCHIVE:		$Archive: /Project/Units/SaVic/SavicDll/CMotionDetection.cpp $
...
//////////////////////////////////////////////////////////////////////
CMotionDetection::CMotionDetection(CBT878 *pBT878, const CString& sIniName)
{
	int nI;
	...
	m_nCaptureSizeH = ReadConfigurationInt(_T("MotionDetection"), _T("CaptureSizeH"), CAPTURE_SIZE_H, m_sIniName);
	m_nCaptureSizeV = ReadConfigurationInt(_T("MotionDetection"), _T("CaptureSizeV"), CAPTURE_SIZE_V, m_sIniName);
	...
	m_pPerf2 = new CWK_PerfMon(_T("Compare"));
}
...
BOOL CMotionDetection::Init(CCodec* pCodec)
{
	...
	if (m_pBT878)
	{
		// Color Output format auf YUV422 setzen
		m_pBT878->BT878Out(BT878_COLOR_FMT, BT878_YUV2<<4 | BT878_YUV2);

		// Dither, NoVBIOdd, NoVBIEven, NoVDFCOdd, NoVDFCEven		
...
BOOL CMotionDetection::MotionCheck(CAPTURE_STRUCT& Capture, CMDPoints &Points)
{
	...
	{
		ML_TRACE_ERROR(_T("CMotionDetection::MotionCheck\tWrong Camera (%d)\n"), wSubID+1);
		return FALSE;
	}
	...
	if (m_bDIBIndication[wSubID])
	{
		...
		if (hDIB)
		{
			// ... und ab damit
			if (m_pCodec)
				m_pCodec->OnReceivedMotionDIB(wSubID, hDIB);
			GlobalFree(hDIB);
		}
	}
...
BOOL CMotionDetection::CompareImages(WORD wSubID, CMDPoints &Points)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::CompareImages\tWrong Camera (%d)\n"), wSubID+1);
		return FALSE;
	}
...
void CMotionDetection::LoadParameter(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::LoadParameter\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
	...
	// Histogramm? Ja/Nein.
	m_bHistogramm[wSubID]	= ReadConfigurationInt(szBuffer, _T("Histogramm"), 0, m_sIniName);	

	// Filter? Ja/Nein.
	m_bFilter[wSubID]		= ReadConfigurationInt(szBuffer, _T("TPFilter"), 1, m_sIniName);	

	// Mindest-, und Maximalzeit zwischen zwei Differentbildern
	m_nMinTime[wSubID] = ReadConfigurationInt(szBuffer, _T("MinTime"), 250,   m_sIniName); 
	m_nMaxTime[wSubID] = ReadConfigurationInt(szBuffer, _T("MaxTime"), 30000, m_sIniName); 
...
{									  
	...
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::SaveMask\tWrong Camera (%d)\n"), wSubID+1);
		return FALSE;
	}
	...
	if (eMaskType == AM_ADAPTIVE)
		sFileName.Format(_T("%sMDA%02u.msk"), m_sMaskPath, wSubID);
	else if (eMaskType == AM_PERMANENT)
		sFileName.Format(_T("%sMDP%02u.msk"), m_sMaskPath, wSubID);
	else
		ML_TRACE_ERROR(_T("SaveMask: Unknown Masktype (0x%x)\n"), eMaskType);
...
BOOL CMotionDetection::LoadMask(WORD wSubID)
{
	...
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::LoadMask\tWrong Camera (%d)\n"), wSubID+1);
		return FALSE;
	}
	...
	CString sFileName;
	sFileName.Format(_T("%sMotionD%u.msk"), m_sMaskPath, wSubID);
...
BOOL CMotionDetection::LoadMaskX(WORD wSubID)
{
	...
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::LoadMaskX\tWrong Camera (%d)\n"), wSubID+1);
		return FALSE;
	}
		...
		if (SetMask(wSubID, MaskAdaptive))
		{
			...
			if (MaskPermanent.LoadMaskFromFile(sFileName))
			{
				if (SetMask(wSubID, MaskPermanent))
					...
					ML_TRACE_ERROR(_T("LoadMaskX: Failed to set permanent mask\n"));
			}
			else
				ML_TRACE_ERROR(_T("LoadMaskX: Failed to load permanent mask\n"));
		}
		else
			ML_TRACE_ERROR(_T("LoadMaskX: Failed to set adaptive mask\n"));
	}
	else
		ML_TRACE_ERROR(_T("LoadMaskX: Failed to load adaptive mask\n"));
...
void CMotionDetection::ClearMask(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::ClearMask\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
void CMotionDetection::ClearAdaptiveMask(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::ClearAdaptiveMask\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
void CMotionDetection::ClearPermanentMask(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::ClearPermanentMask\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
void CMotionDetection::InvertPermanentMask(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::InvertPermanentMask\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
void CMotionDetection::InvertAdaptiveMask(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::InvertAdaptiveMask\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
void CMotionDetection::ToggleAdaptiveMaskActivation(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::ToggleAdaptiveMaskActivation\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
void CMotionDetection::ActivateAdaptiveMask(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::ActivateAdaptiveMask\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
void CMotionDetection::DeactivateAdaptiveMask(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::DeactivateAdaptiveMask\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
void CMotionDetection::ToggleMDActivation(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::ToggleMDActivation\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
BOOL CMotionDetection::ActivateMotionDetection(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::ActivateMotionDetection\tWrong Camera (%d)\n"), wSubID+1);
		return FALSE;
	}
...
BOOL CMotionDetection::DeactivateMotionDetection(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::DeactivateMotionDetection\tWrong Camera (%d)\n"), wSubID+1);
		return FALSE;
	}
...
void CMotionDetection::ToggleHistogramm(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::ToggleHistogramm\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
void CMotionDetection::ActivateHistogramm(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::ActivateHistogramm\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
	...
	_TCHAR szBuffer[255];
	wsprintf(szBuffer, _T("Mapping\\Source%02u\\MD"), wSubID);
	WriteConfigurationInt(szBuffer, _T("Histogramm"), m_bHistogramm[wSubID], m_sIniName);	
}
...
void CMotionDetection::DeactivateHistogramm(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::DeactivateHistogramm\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
void CMotionDetection::ToggleFilter(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::ToggleFilter\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
void CMotionDetection::ActivateFilter(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::ActivateFilter\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
	...
	_TCHAR szBuffer[255];
	wsprintf(szBuffer, _T("Mapping\\Source%02u\\MD"), wSubID);
	WriteConfigurationInt(szBuffer, _T("TPFilter"), m_bFilter[wSubID], m_sIniName);	
}
...
void CMotionDetection::DeactivateFilter(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::DeactivateFilter\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
void CMotionDetection::ShowOriginalFrame(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::ShowOriginalFrame\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
void CMotionDetection::ShowDifferenceFrame(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::ShowDifferenceFrame\tWrong Camera (%d)\n"), wSubID+1);
		return;
	}
...
BOOL CMotionDetection::SetMDMaskSensitivity(WORD wSubID, const CString &sLevel)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::SetMDMaskSensitivity\tWrong Camera (%d)\n"), wSubID+1);
		return FALSE;
	}
...
BOOL CMotionDetection::SetMDAlarmSensitivity(WORD wSubID, const CString &sLevel)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::SetMDAlarmSensitivity\tWrong Camera (%d)\n"), wSubID+1);
		return FALSE;
	}
...
CString CMotionDetection::GetMDMaskSensitivity(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::GetMDMaskSensitivity\tWrong Camera (%d)\n"), wSubID+1);
		return CSD_NORMAL;
	}

	_TCHAR szSection[255] = {0};
	_TCHAR szBuffer[255]	= {0};
	wsprintf(szSection, _T("Mapping\\Source%02u\\MD"), wSubID);

	ReadConfigurationString(szSection, _T("MaskSensitivity"), CSD_NORMAL, szBuffer, 16, m_sIniName);
	return szBuffer;
}
...
CString CMotionDetection::GetMDAlarmSensitivity(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::GetMDAlarmSensitivity\tWrong Camera (%d)\n"), wSubID+1);
		return CSD_NORMAL;
	}
...
BOOL CMotionDetection::EnableDIBIndication(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::EnableDIBIndication\tWrong Camera (%d)\n"), wSubID+1);
		return FALSE;
	}
...
BOOL CMotionDetection::DisableDIBIndication(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::DisableDIBIndication\tWrong Camera (%d)\n"), wSubID+1);
		return FALSE;
	}
...
HANDLE CMotionDetection::CreateDIB(WORD wSubID)
{
	if (wSubID >= MAX_CAMERAS)
	{
		ML_TRACE_ERROR(_T("CMotionDetection::CreateDIB\tWrong Camera (%d)\n"), wSubID+1);
		return NULL;
	}
...
/////////////////////////////////////////////////////////////////////////////
void CMotionDetection::CreateRiscPrg(const CRect &rc)   
{
}

/////////////////////////////////////////////////////////////////////////////
BOOL CMotionDetection::EnableRisc()
{
	BOOL bResult = FALSE;
...
/////////////////////////////////////////////////////////////////////////////
BOOL CMotionDetection::DisableRisc()
{
	BOOL bResult = FALSE;
	if (m_pBT878)
	{
