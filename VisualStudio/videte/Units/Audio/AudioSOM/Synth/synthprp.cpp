...
HRESULT CSynthProperties::OnConnect(IUnknown *pUnknown)
{
    ...
    HRESULT hr = pUnknown->QueryInterface(IID_ISynthesizer,(void **)&m_pSynth);
    if (FAILED(hr)) {
        return hr;
    }
...
HRESULT CSynthProperties::OnDisconnect()
{
    ...
    m_pSynth->put_OffsetDegree(0);
    m_pSynth->put_TimePercent(50);
                                      ...
                                      , LPARAM lParam)
{
    ...
    case WM_PROPERTYPAGE_ENABLE:
        ...
        EnableWindow (GetDlgItem (hwnd, IDC_SAMPLINGFREQ48), (BOOL) lParam);
        EnableWindow (GetDlgItem (hwnd, IDC_SAMPLINGFREQ96), (BOOL) lParam);
        EnableWindow (GetDlgItem (hwnd, IDC_SAMPLINGFREQ192), (BOOL) lParam);

        EnableWindow (GetDlgItem (hwnd, IDC_BITSPERSAMPLE8),  (BOOL) lParam);
        EnableWindow (GetDlgItem (hwnd, IDC_BITSPERSAMPLE16), (BOOL) lParam);
        EnableWindow (GetDlgItem (hwnd, IDC_BITSPERSAMPLE24), (BOOL) lParam);
    ...
    case WM_COMMAND:
        ...
        case IDC_SAMPLINGFREQ44:
            ...
            SetDirty();
            break;
        case IDC_SAMPLINGFREQ48:
            m_pSynth->put_SamplesPerSec(48000);
            RecalcFreqSlider();
            SetDirty();
            break;
        case IDC_SAMPLINGFREQ96:
            m_pSynth->put_SamplesPerSec(96000);
            RecalcFreqSlider();
            SetDirty();
            break;
        case IDC_SAMPLINGFREQ192:
            m_pSynth->put_SamplesPerSec(192000);
            RecalcFreqSlider();
            SetDirty();
            break;
        ...
        case IDC_BITSPERSAMPLE16:
            ...
            SetDirty();
            break;
        case IDC_BITSPERSAMPLE24:
            m_pSynth->put_BitsPerSample(24);
            SetDirty();
            break;
        ...
        case IDC_OF_IEEE_FLOAT:
            hr = m_pSynth->put_OutputFormat(SYNTH_OF_IEEE_FLOAT);
            if (SUCCEEDED(hr)) {
                EnableBitsPerSampleRadioButtons(hwnd, FALSE);
            } else {
                MessageBeep(MB_ICONASTERISK);
...
void CSynthProperties::InitPropertiesDialog(HWND hwndParent)
{
    ...
    switch (m_iSamplesPerSecOriginal) 
    {
        ...
        case  48000: i = IDC_SAMPLINGFREQ48; break;
        case  96000: i = IDC_SAMPLINGFREQ96; break;
        case 192000: i = IDC_SAMPLINGFREQ192; break;
        default:
            ASSERT(0);  break;
    }
    CheckRadioButton(hwndParent, IDC_SAMPLINGFREQ11, IDC_SAMPLINGFREQ192, i);
    ...
    if(   SYNTH_OF_MS_ADPCM   == m_OutputFormat
       || SYNTH_OF_IEEE_FLOAT == m_OutputFormat) 
    {
...
CSynthProperties::RecalcFreqSlider(void)
{
    ...
    SendMessage(m_hwndFreqSlider, TBM_SETRANGEMIN, TRUE, 0);
    SendMessage(m_hwndFreqSlider, TBM_SETRANGEMAX, TRUE, iMaxFreq);
...
void CSynthProperties::OnFreqSliderNotification(WPARAM wParam, WORD wPosition)
{
    int MaxFreq;
    int Freq;
    int SliderPos;
...
void CSynthProperties::SetSweepRange(int MaxFreq)
{
...
void CSynthProperties::SetDirty()
{
    m_bDirty = TRUE;
    if (m_pPageSite)
...
void CSynthProperties::EnableBitsPerSampleRadioButtons(HWND hwndParent, BOOL fEnable)
{
    ...
    EnableWindow (GetDlgItem (hwndParent, IDC_BITSPERSAMPLE24), fEnable);
}
