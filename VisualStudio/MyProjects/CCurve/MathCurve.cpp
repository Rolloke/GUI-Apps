...
double CMathCurve::Slope(double x, double &y, int *startindex)
{
   ...
   if (dx == 0) return 0;
   dy = GetX_Value(*startindex) - GetX_Value(*startindex-1);
   return dy / dx;
}
...
void CMathCurve::DivideUnit(char * pszUnit)
{
   if (pszUnit && m_pszUnity)
   {
      char  szAbove[UNIT_STR_LEN] = "";
      char  szUnder[UNIT_STR_LEN] = "";
...
void CMathCurve::MultiplyUnit(char * pszUnit)
{
   if (pszUnit && m_pszUnity)
   {
      char  szAbove[UNIT_STR_LEN] = "";
...
bool CMathCurve::MultiplyUnitPart(char * pszPart, char * pszUnit)
{
   if ((pszPart != NULL) && (pszUnit != NULL))
   {
      ...
      {
         if (strlen(pszPart)>0) strncat(pszPart, "*", UNIT_STR_LEN);
...
bool CMathCurve::DivideUnitPart(char * pszPart, char * pszUnit)
{
   if ((pszPart != NULL) && (pszUnit != NULL))
   {
      char *ptr = strstr(pszPart, pszUnit);
...
void CMathCurve::SplitUnit(char * pszUnit, char * pszAbove, char * pszUnder)
{
   char *ptr = strstr(pszUnit, "/");
   if (ptr)
   {
