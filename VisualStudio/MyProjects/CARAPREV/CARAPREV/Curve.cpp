...
CCurve::CCurve(CCurve *pc)
{
   ...
   if (pc)
   {
      ...
      m_dOrg      = pc->m_dOrg;
      m_dStep     = pc->m_dStep;
      if (pc->m_pGetCrvParamFkt)
      {
         CF_GET_CURVE_PARAMS gcp = {CF_COPY_CURVE, 0, this, NULL, pc};
         m_pGetCrvParamFkt = pc->m_pGetCrvParamFkt;
...
*/
#else
   static const TCHAR *szErrorText[] = 
   {                             // Error
...
#else
      MessageBox((HWND) NULL, _T("Detected Memory Leaks on CCurve-Heap !"), _T("HeapWatch"), MB_OK|MB_ICONEXCLAMATION);
#endif
   }
   if (gm_hHeap)
   {
...
#else
      TCHAR text[64];
      wsprintf(text, _T("Number of Created Objects on CCurve-Heap %d :  %d"), gm_hHeap, gm_nInstances);
      MessageBox((HWND) NULL, text, _T("HeapWatch"), MB_OK|MB_ICONEXCLAMATION);
#endif
      return true;
   }
   return false;
...
#else
      MessageBox((HWND) NULL, _T("Heap Destruction failure !"), _T("HeapWatch"), MB_OK|MB_ICONEXCLAMATION);
#endif
   }
}
...
void CCurve::Init()
{
   ...
   memset(&m_ParameterEx, 0, sizeof(BSParameterEx));
   m_nNum                   = 0;
   m_pArray                 = NULL;
...
bool CCurve::SetSize(int n)
{
   if ((   m_Parameter.format == CF_ARBITRARY
	   ...
	   && m_pGetCrvParamFkt)
   {
       CF_GET_CURVE_PARAMS gcp = {CF_SET_SIZE, n, this, NULL, m_pGetCrvParam};
...
void CCurve::Write(HANDLE hfile)
{
   ...
   if (   m_Parameter.format == CF_ARBITRARY
	    || m_Parameter.format == CF_ARBITRARY_CONTR)
   {
	   CF_GET_CURVE_PARAMS gcp = {CF_WRITE_FILE, 0, this, NULL, m_pGetCrvParam};
	   m_pGetCrvParamFkt(&gcp) != 0 ? true : false;
   }
   else
   {
...
bool CCurve::Read(HANDLE hfile)
{
   ...
   if (   m_Parameter.format == CF_ARBITRARY
	    || m_Parameter.format == CF_ARBITRARY_CONTR)
   {
	   CF_GET_CURVE_PARAMS gcp = {CF_READ_FILE, 0, this, NULL, m_pGetCrvParam};
	   m_pGetCrvParamFkt(&gcp) != 0 ? true : false;
   }
   else
   {
   ...
   if (size)
   {
      m_pszDescription = new TCHAR[size+1];
   ...
   if (m_Parameter.locus)
   {
      ...
      if (size)
      {
         m_pszUnitLocus = new TCHAR[size+1];
...
void CCurve::SetDescription(const TCHAR *s)
{
   if (m_pszDescription) delete[] m_pszDescription;
   m_pszDescription = NULL;
   if (s)
   {
      m_pszDescription = new TCHAR[_tcsclen(s)+1];
      strcpy(m_pszDescription, s);
   }
}

void CCurve::SetXUnit(const TCHAR *s)
{
   if (m_pszUnitx) delete[] m_pszUnitx;
   m_pszUnitx = NULL;
   if (s)
   {
      m_pszUnitx = new TCHAR[_tcsclen(s)+1];
      strcpy(m_pszUnitx, s);
   }
}

void CCurve::SetYUnit(const TCHAR *s)
{
   if (m_pszUnity) delete[] m_pszUnity;
   m_pszUnity = NULL;
   if (s)
   {
      m_pszUnity = new TCHAR[_tcsclen(s)+1];
      strcpy(m_pszUnity, s);
   }
}

void CCurve::SetLocusUnit(const TCHAR *s)
{
   if (m_pszUnitLocus) delete[] m_pszUnitLocus;
   m_pszUnitLocus = NULL;
   if (s)
   {
      m_pszUnitLocus = new TCHAR[_tcsclen(s)+1];
...
void CCurve::CopyValues(CCurve *pc)
{
   ...
   {
      if (  pc->m_Parameter.format == CF_ARBITRARY
         || pc->m_Parameter.format == CF_ARBITRARY_CONTR)
      {
         if (m_pGetCrvParamFkt)
         {
            CF_GET_CURVE_PARAMS gcp = {CF_COPY_VALUES, 0, this, NULL, pc};
            VERIFY(m_pGetCrvParamFkt(&gcp)!=0);
         }
      }
      else if (pc->m_Parameter.format == m_Parameter.format)
      {
