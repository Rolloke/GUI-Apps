...
int StaticDialog::doModal(int dialogID)
{
...
{
    int fLen = SendDlgItemMessage(_hSelf, nID, WM_GETTEXTLENGTH, 0, 0);
    if (fLen)
    {
        ++fLen;
        aText.resize(fLen);
...
{
    return SetDlgItemInt(_hSelf, aID, aValue, aSigned);
}
...
{
    return ::SendMessage(mHwnd, TVM_DELETEITEM, 0, (LPARAM)parent);
}
