...
{
  QString fS = QString::number(value, base);
  getBuffer( _length = fS.size() );
  if ( _buffer != NULL )
    strcpy( _buffer, fS.toStdString().c_str() );
}
...
{
    QString fS = QString::number(value, 'f', decimalPlaces);
    getBuffer( _length = fS.size() );
    if ( _buffer != NULL )
      strcpy( _buffer, fS.toStdString().c_str() );
}
...
{
  for ( unsigned int i = 0; i < _length; i++ )
    _buffer[ i ] = (char)tolower( _buffer[ i ] );
}
